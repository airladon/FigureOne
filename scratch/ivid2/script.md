...
So for example, if you take the ratio of sides sA and sB form the bottom triangle, the factor s cancels and we are left with the ratio A over B from the top triangle, and you can do this for any set of sides.
...
Now specifically the trigonometric functions come from studying the relationship between sides and angles in a right angle triangle which by definition has one angle as a right angle. This means when we define one more angle theta, then because angles in a triangle always add to 180º, we are effectively defining all three angles.
...
# Ratios
So that means for instance that the ratio between the side opposite theta and the hypotenuse is constant, or will have the same value for all right triangles with the same angle theta.
...
Now, these ratios are constant when theta is constant, but when we change theta, we can see the ratio values change. This means each ratio is dependent on, or a function of theta. 

And these are the trigonometric functions.

# Names
## Lines
Each of these functions have abreviated names that come directly from Latin, and are not necessarily intuitive. However, the names are all derived from just three different lines related to a circle, and knowing these lines can help make some sense of the names.
...
We start with a circle or radius 1, also called a unit circle, because as you will see, if we can get that value of 1 into the denominator of these equations, it will simplify them. 
...
We start with a quarter circle of radius 1, around which we construct a right angle triangle such that the hypotenuse touches the circle at some angle theta, and is tangent to it.

## Tan Sec
And so we can call the side opposite theta in the lower triangle the tangent, which is a function that describes how the line length changes with theta, although this is often abreviated simply as tan theta or tan.

We also have a line that cuts through the circle, and is a secant line, which we will abbreviate just as sec.

These two sides form a right angle triangle with the radius, which is adjacent to theta and has a value of 1, and so side ratios with the adjcent side in the denominator can be expressed simply as one of the two sides we just named.

So, the opposite over the adjacent is tan over one, or simply tan. Similarly the hypotenuse over adjacent is the sec.
...
## Cot CSC
This triangle has a tangent line, but this time it is opposite the *complementary angle of theta*, and so its value is the *tangent of the complementary angle*. Therefore we will call it the complementary tangent, or cotangent or cot for short. So now we could say, ah well lets make our ratio tan (comp)

Similarly the we have a complimentary secant line, which we call the cosec or csc for short.

Out third side has a value 1 and is this time opposite theta, so we can fill in the two ratios with the opposite side in the denominator.
...
So the opposite over hypotenuse ratio is the sine.
...
And so, that was kind of a long winded way to say, each of these ratios have a special abreviated name that comes from Latin and the relationship between circles, triangles and right angles.
