...
const { segmentPolyline, getPolygonCorners } = Fig.tools.morph;
...
const [sincPoints] = segmentPolyline({
  polyline: range(-0.8, 0.8, 0.02).map(x => [x, sinc(x, 0.5, 20)]),
  num: n,
  size: 0.01,
});

// Generate a line of points along a square
const [squarePoints] = segmentPolyline({
...
const [circlePoints] = segmentPolyline({
  polyline: getPolygonCorners({ radius: 0.5, sides: 50, rotation: Math.PI / 4, direction: 1 }),
  num: n,
  size: 0.01,
  close: true,
});
