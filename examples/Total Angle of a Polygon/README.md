...
The code has three main parts. The first part defines the geometry and a number of helper functions that will be reused several times, the second part adds the shape and the equation to the figure, and the third part sets the behaviors when the equations is touched.
...
// with the "highlight" rectangle, and animate the corresponding angle on the figure
