...
describe('Animator API', () => {
  ...
  test('Basic', () => {
    ...
    element.animator
      ...
      .start();
    element.animator.nextFrame(100);
    element.animator.nextFrame(100.1);
    expect(element.getPosition().round()).toEqual(point(0.1));

    let remaining = element.animator.nextFrame(101.1);
    expect(element.getPosition().round()).toEqual(point(1.1));
    expect(math.round(remaining)).toBe(0);

    remaining = element.animator.nextFrame(102.1);
