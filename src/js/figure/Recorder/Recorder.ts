...
// @ts-ignore - webpack worker-loader provides default export
...
function getIndexRangeForTime(
  ...
  const startTime = parseFloat(recordedData[startSearch][0] as any);
  if (time === startTime) {
  ...
  const endTime = parseFloat(recordedData[endSearch][0] as any);
  if (time === endTime) {
  ...
  const midTime = parseFloat(recordedData[midSearch][0] as any);
  if (time === midTime) {
...
function getMostRecentForTime(
  ...
  let latestEventOrState: any = null;
...
class Recorder {
  ...
  loadEvents(
    encodedEventsList: Record<string, any> | Array<TypeEvents>,
    isMinified: boolean = true,
  ) {
    const lists: Record<string, any> = this.decodeEvents(encodedEventsList as any, isMinified);
  ...
  encodeStates(
    ...
    precision: number | null = this.precision,
  ) {
    let states;
    if (asObject) {
  ...
  decodeStates(
    ...
    let statesToUse: any = statesIn;
    if (isMinified) {
      statesToUse = unminify(statesIn as any);
    }
    if (asObject) {
  ...
  clear(startTime: number, stopTime: number) {
    Object.keys(this.events).forEach((eventName) => {
      const event = this.events[eventName];
      event.list = this.clearListOrDiffs(event.list, startTime, stopTime) as TypeEvents;
    });
    ...
    if (message === 'cache') {
      ...
      if (this.recordingStates) {
        if (this.duration % 1 > 0) {
          ...
          this.states.diffs.push([Math.ceil(this.duration), ref, duplicate(diff), 0] as any);
  ...
  mergeEventsCache() {
    const allEventNames: Record<string, any> = {};
    Object.keys(this.events).forEach((eventName) => {
    ...
    Object.keys(allEventNames).forEach((eventName) => {
      ...
      this.events[eventName].list = merged as TypeEvents;
    });
  }
  ...
  addEventType(
    eventName: string,
    playbackAction: (payload: any) => void,
  ...
  recordState(state: Record<string, any>, time: number = this.getCurrentTime()) {
    const now = time;
  ...
  recordEvent(
    eventName: string,
    payload: Array<string | number | Record<string, any>>,
...
${cursorData}
  ...
  showEvent(eventName: string, precision: number = 2, space: number = 0) {
    ...
    this.events[eventName].list.forEach((event) => {
      ...
      if (Array.isArray(payload)) {
        payloadStr = '';
        const payloadOut: Array<string | number> = [];
        payload.forEach((item) => {
          if (typeof item === 'string') {
            payloadOut.push(`'${item}'`);
          } else {
            payloadOut.push(round(item as number, precision));
          }
        });
  ...
  encodeCursorEvent(
    ...
    let lastDeltaTime = 0;
    let lastValues: Array<number> | null = null;
    this.events[eventName].list.forEach((event) => {
      const [time, payload] = event;
      const deltaTime = round(time - firstTime, timePrecision);
      const values = (payload as any[]).map((p: any) => round(p, valuePrecision));
      if (lastValues == null) {
        lastValues = values;
      ...
      for (let i = 0; i < values.length; i += 1) {
        deltaValues[i] = Math.round((values[i] - (lastValues as any)[i]) * 10 ** valuePrecision);
      }
      out.push(
  ...
  loadEventData(
    ...
    if (decode) {
      ...
      const timeConvertedData = data.map((d: any) => [this.convertTime(d[0]), d[1], 0]);
  ...
  setToTime(timeIn: number, force: boolean = false) {
    ...
    if (this.stateIndex !== -1) {
      [stateTime, , , stateTimeCount] = this.states.diffs[this.stateIndex] as any;
    }
    ...
    const eventsToSetBeforeState: any[] = [];
    const eventsToSetAfterState: any[] = [];
    ...
    // timer.stamp('m3');
    const sortTimes = (arrayToSort: any[]) => arrayToSort.sort((a: any, b: any) => {
  ...
  getCursorState(atTime: number) {
    ...
    let touchUp: boolean | null = null;
    let showCursor: boolean | null = null;
    let cursorPosition: Point | null = null;
    let cursorTime: number | null = null;
    let cursorTimeCount: number | null = null;
    if (touchEvent != null) {
  ...
  getPlaySettings() {
    let onResume: Record<string, any> = {
      how: 'instant',
      maxDuration: 6,
      velocity: {
        position: 2,
    ...
    if (typeof this.settings.play === 'string') {
      ...
      onResume = joinObjects<any>({}, onResume, this.settings.play);
    }
    if (onResume.how === 'dissolve') {
      ...
      if (onResume.duration === 0) {
        ...
        onResume.duration = joinObjects<any>({}, defaultDuration, onResume.duration);
      }
  ...
  startAudioPlayback(fromTime: number, callback: (() => void) | null = null) {
    const { audio } = this;
    if (audio != null) {
  ...
  getNextEvent() {
    ...
    let nextTimeCount: number | null = null;
    this.eventsToPlay.forEach((eventName) => {
      if (
