...
describe('Figure Primitives TextLine', () => {
  ...
  test('basic modifier', () => {
    ...
    const t = figure.elements._t.drawingObject;
    expect(t.text.length).toBe(2);
    expect(t.text[0].text).toBe('a ');
    expect(t.text[1].text).toBe('gg');
    const tr = figure.elements._t.getBoundingRect('figure');
  ...
  describe('All Options', () => {
    ...
    beforeEach(() => {
      loadText('allOptions');
      ({ text } = figure.elements._t.drawingObject);
      lineSpace = -0.2;
    });
    test('color', () => {
...
describe('Text Borders', () => {
  ...
  test('Simple', () => {
    addElement('simple');
    expect(round(td.text[0].textBorder, 3)).toEqual(round(getPoints([
      [0, bot], [w, bot], [w, top], [0, top],
    ]), 3));
    expect(round(td.text[1].textBorder, 3)).toEqual(round(getPoints([
      [0, -l + bot], [w, -l + bot], [w, -l + top], [0, -l + top],
    ]), 3));
    expect(round(td.text[0].textBorderBuffer, 3)).toEqual(round(getPoints([
      [0, bot], [w, bot], [w, top], [0, top],
    ]), 3));
    expect(round(td.text[1].textBorderBuffer, 3)).toEqual(round(getPoints([
      [0, -l + bot], [w, -l + bot], [w, -l + top], [0, -l + top],
    ]), 3));
