...
export default class FigureElementPrimitiveGesture extends FigureElementPrimitive {
  ...
  reset(exceptDistance: boolean = false) {
    ...
    this.zoom.mag = 1;
    this.zoom.cumAngle = new Point(0, 0) as any;
   ...
   * reset.
   */
  override setScene(scene: Scene | OBJ_Scene) {
  ...
  updateZoom(mag: number, zoomPosition: Point, distance: number = 0, angle: number = 0) {
    ...
    this.setPanOffset(this.pan.offset.add(this.pan.delta));
    (this.zoom as any).cumAngle += angle - this.zoom.last.angle;
  }
  ...
  setZoomOptions(options: OBJ_ZoomOptions) {
    if (options.min !== undefined) { this.zoom.min = options.min as any; }
    if (options.max !== undefined) { this.zoom.max = options.max as any; }
  ...
  setPanOptions(options: OBJ_PanOptions) {
    if (options.left !== undefined) { this.pan.left = options.left as any; }
    if (options.bottom !== undefined) { this.pan.bottom = options.bottom as any; }
    if (options.top !== undefined) { this.pan.top = options.top as any; }
    if (options.right !== undefined) { this.pan.right = options.right as any; }
    if (options.wheelSensitivity !== undefined) {
  ...
  setZoom(
    ...
    if (offset == null && position == null) {
      ...
      this._setZoomPan(mag, offset as Point, new Point(0, 0));
    } else if (position != null) {
      this.updateZoom(mag, position as Point, 0, 0);
    }
  ...
  override setFigure(figure: OBJ_FigureForElement) {
    this.figure = figure;
    if (figure != null) {
