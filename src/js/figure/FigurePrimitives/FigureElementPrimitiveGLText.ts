...
export default class FigureElementPrimitiveGLText extends FigureElementPrimitive {
  ...
  override drawingObject!: GLObject;
  ...
  setup(options: OBJ_Text_Fixed) {
    ...
    if (typeof options.text[0] === 'object') {
      this.text = [(options.text[0] as any).text];
    } else {
      this.text = options.text as Array<string>;
    }
    ...
    this.adjustments = options.adjustments as any;
    ...
    this.createAtlas((options as any).scene);
  ...
  createAtlas(sceneIn: Scene = this.getScene() as Scene) {
    let scene;
    ...
    this.atlas = (this.drawingObject as any).webgl.getAtlas({
      scene,
      font: this.font,
    });
  ...
  showMap(dimension: number = 1) {
    ...
    (this.drawingObject as any).updateTextureMap(points);
  ...
  measureAndAlignText() {
    ...
    for (let t = 0; t < this.text.length; t += 1) {
      ...
      if (this.font.underline !== false) {
        const uDescent = (this.font.underline as any).descent;
        const uWidth = (this.font.underline as any).width;
    ...
    (this.drawingObject as any).updateTextureMap(texCoords.map(v => v / this.atlas.dimension));
  ...
  setFont(font: OBJ_Font) {
    const newFont = joinObjects<any>({}, this.font.definition(), font);
