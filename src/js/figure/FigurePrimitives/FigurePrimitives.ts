...
// function processOptions(...optionsIn: Array<Record<string, any>>) {
...
function setupPulse(element: FigureElement, options: Record<string, any>) {
  if (options.pulse != null) {
    if (
      ...
      if (typeof options.pulse === 'number') {
        // eslint-disable-next-line no-param-reassign
        (element.pulseDefault as any).scale = options.pulse;
      } else {
        // eslint-disable-next-line no-param-reassign
        element.pulseDefault = joinObjects<any>({}, element.pulseDefault, options.pulse);
      }
    }
  }
}
...
export default class FigurePrimitives {
  ...
  gl(...optionsIn: Array<OBJ_GenericGL>) {
    ...
    const isValidMask = (m: any) => m != null && m.src != null && m.src !== '';
    ...
    const hasMask = rawMasks.some(isValidMask);
    const maskList = hasMask ? rawMasks : [];
    const numMasks = maskList.length;
    const NUM_TINTS = numMasks * CHANNELS_PER_MASK;
    ...
    if (hasMask) {
      const { gl } = this.webgl[0];
      const maxUnits = gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS);
      if (typeof maxUnits === 'number' && maxUnits > 0 && numMasks + 1 > maxUnits) {
        throw new Error(`FigureOne gl: ${numMasks} mask textures (plus the base texture) exceed this device's ${maxUnits} texture units. Use fewer masks.`);
      }
    }
    const defaultOptions: any = {
      ...
      texture: {
        ...
        repeat: false,
        onLoad: this.animateNextFrame as () => void,
        coords: [],
        loadColor: [0, 0, 1, 0.5],
      },
      tints: [],
    ...
    const options = joinObjects<any>({}, defaultOptions, oIn);
    ...
    if (options.attributes != null) {
      options.attributes.forEach((buffer: any) => {
        ...
        const b = joinObjects<any>({}, defaultAttribute, buffer);
    ...
    if (options.uniforms != null) {
      options.uniforms.forEach((uniform: any) => {
        const defaultUniform = {
          type: 'FLOAT',
          length: 1,
        };
        const u = joinObjects<any>({}, defaultUniform, uniform);
        glObject.addUniform(
    ...
    element.custom.updateAttribute =
      (element.drawingObject as any).updateAttribute.bind(element.drawingObject);
  ...
  generic3(...optionsIn: Array<OBJ_Generic3>) {
    ...
    const processOptions = (o: Record<string, any>, u: any) => {
      if (o.usage == null) {
        o.usage = u;
      }
      if (o.points != null) {
      ...
      if (o.copy != null) {
        ...
        if (o.colors != null) {
          ...
          for (let i = 0; i < count; i += 1) {
            out.push(...o.colors.map((c: any) => c.slice()));
          }
          o.colors = out;
        }
    ...
    element.custom.updateGeneric3 = function update(updateOptions: {
      ...
      processOptions(o, u);
      if ((o as any).vertices) {
        element.custom.updateVertices((o as any).vertices.data);
        if ((o as any).texture != null && (o as any).texture.coords != null) {
          (element.drawingObject as any).updateTextureMap((o as any).texture.coords);
        } else if ((o as any).texture != null && (element.drawingObject as any).texture != null) {
          if ((element.drawingObject as any).texture.points != null) {
            (element.drawingObject as any).updateTextureMap((element.drawingObject as any).texture.points);
          } else {
            (element.drawingObject as any).updateTextureMap([]);
          }
        }
      }
      if ((o as any).normals && (element.drawingObject as any).attributes.a_normal != null) {
        element.custom.updateAttribute('a_normal', (o as any).normals.data);
      }
      if ((o as any).colors) {
        element.custom.updateAttribute('a_color', (o as any).colors);
      }
      if ((o as any).drawType != null) {
        (element.drawingObject as any).setPrimitive((o as any).drawType.toUpperCase());
      }
      if ((o as any).touchScale != null) {
        (element as any).touchScale = getScale((o as any).touchScale);
      }
    };
  ...
  generic3DBase(
    ...
    getPointsFn: (o: Record<string, any>) => ([Array<Point>, Array<Point>] | [Array<Point>]),
  ) {
    const options = joinObjects<any>({}, defaultOptions, optionsIn);
    const element = this.generic3(joinObjects<any>(
      {},
      options,
      {
        points: [],
    ...
    element.custom.updatePoints = (updateOptions: Record<string, any>) => {
      ...
      if (o.lines == null || o.lines === false) {
        element.custom.updateGeneric3(joinObjects<any>({}, o, {
          points,
          normals,
        }));
      } else {
  ...
  morph(...optionsIn: Array<OBJ_Morph>) {
    ...
    let fragmentShader: string = 'simple';
    ...
    const shapeNameMap: Record<string, any> = {};
    if (options.names != null) {
      options.names.forEach((name: string, index: number) => { shapeNameMap[name] = index; });
    }
    options.points.forEach((points: any, index: number) => {
      ...
      const b = joinObjects<any>({}, defaultBuffer);
      glObject.addAttribute(
    ...
    if (colorVertex) {
      options.color.forEach((colorsIn: any, index: number) => {
        let colors = colorsIn;
        if (colors.length === 4) {
    ...
    element.fnMap.add('_morphCallback', (percentage: number, customProperties: Record<string, any>) => {
      const { start, target } = customProperties;
      element.setPointsBetween(start, target, percentage);
    });
    element.animations.morph = (...opt: any[]) => {
      const o = joinObjects<any>({}, {
  ...
  polyline(...optionsIn: Array<OBJ_Polyline>) {
    const options = joinObjects<any>({}, ...optionsIn);
    ...
    element.custom.updatePoints = (updateOptions: OBJ_Polyline) => {
      const [o, points, drawBorder, drawBorderBuffer, drawType] =
        this.getPolylineTris(joinObjects<any>({}, element.custom.options, updateOptions));
      element.custom.options = o;
      element.custom.updateGeneric(joinObjects<any>({}, o, {
  ...
  getPolygonBorder(optionsIn: OBJ_Polygon_Defined) {
    ...
    let drawBorder;
    if (o.line != null) {
      o.line = joinObjects<any>({}, {
        width: this.defaultLineWidth,
        widthIs: 'mid',
      }, o.line);
      const oLine = o.line as any;
      if (oLine.widthIs === 'inside' && o.direction === 1) {
        oLine.widthIs = 'positive';
      } else if (oLine.widthIs === 'inside' && o.direction === -1) {
        oLine.widthIs = 'negative';
      }
      if (oLine.widthIs === 'outside' && o.direction === 1) {
        oLine.widthIs = 'negative';
      } else if (oLine.widthIs === 'outside' && o.direction === -1) {
        oLine.widthIs = 'positive';
      }
      const { width, widthIs } = oLine;
    ...
    if (drawBorderOffset === 0) {
      ...
      drawBorder = [getPolygonPoints(joinObjects<any>(
  ...
  genericBase(
    ...
    element.custom.updatePoints = (updateOptions: Record<string, any>) => {
      const borderOptions = joinObjects<any>({}, element.custom.options, updateOptions);
      ...
      if (o.line == null) {
        ...
        let bufferOffsetToUse: 'positive' | 'negative' = 'negative';
        ...
        ] = element.custom.getLine(joinObjects<any>(
          {},
          o.line,
          {
            points: border,
  ...
  polygon(...options: Array<OBJ_Polygon>) {
    ...
    (element.drawingObject as any).getPointCountForAngle = (angle: number) => {
    ...
    element.custom.updatePoints(joinObjects<any>({}, ...options));
    return element;
  }
  ...
  star(...options: Array<OBJ_Star>) {
    ...
    element.custom.getBorder = (o: OBJ_Polygon_Defined) => {
      ...
      (result[0] as any).rotation -= Math.PI / 2;
      return result;
    };
    element.custom.getFill = (border: Array<Point>, fillOptions: OBJ_Polygon_Defined) => [
      getTrisFillPolygon(
  ...
  text(...options: Array<OBJ_Text>) {
    ...
    if (o.border != null) {
      element.border = getBorder(o.border) as any;
    }
    if (o.touchBorder != null) {
      element.touchBorder = getBorder(o.touchBorder) as any;
    }
    return element;
  ...
  glText(...options: Array<OBJ_Text>) {
    ...
    const o = this.parseTextOptions(defaultOptions, ...options);
    // const o = joinObjects({}, defaultOptions, ...options);
    o.figureElementPrimitiveCallback = (...fo: any[]) => new (FigureElementPrimitiveGLText as any)(...fo);

    const element: FigureElementPrimitiveGLText = this.gl(o) as any;
    element.setup(o);
    return element;
  }

  elementSetup(element: FigureElementPrimitive, options: Record<string, any>) {
    element.dimColor = this.defaultDimColor.slice();
  ...
  text2d(...options: Array<OBJ_Text>) {
    ...
    const element = new FigureElementPrimitive2DText(this.draw2D[0], o) as any;
  ...
  gesture(...options: Array<OBJ_Gesture>): FigureElementPrimitiveGesture {
    ...
    const combinedOptions = joinObjects<any>({}, ...options);
    for (let i = options.length - 1; i >= 0; i -= 1) {
      if (options[i].scene != null && (options[i] as any).scene instanceof Scene) {
        combinedOptions.scene = (options[i] as any).scene;
        break;
      }
    }
    for (let i = options.length - 1; i >= 0; i -= 1) {
      if ((options[i] as any).changeScene != null && (options[i] as any).changeScene instanceof Scene) {
        combinedOptions.changeScene = (options[i] as any).changeScene;
        break;
      }
    }
    ...
    const element: FigureElementPrimitiveGesture = this.rectangle({
      ...
      yAlign: o.yAlign,
      color: o.color,
      figureElementPrimitiveCallback: (...fo: any[]) => new (FigureElementPrimitiveGesture as any)(...fo),
    } as any) as any;
    element.setup(o);
    element.scene = scene;
  ...
  cameraControl(...options: Array<OBJ_CameraControl>) {
    ...
    for (let i = options.length - 1; i >= 0; i -= 1) {
      if ((options[i] as any).controlScene != null) {
        if ((options[i] as any).controlScene instanceof Scene) {
          element.custom.sceneToChange = (options[i] as any).controlScene;
          i = -1;
        } else if (typeof (options[i] as any).controlScene === 'string') {
          element.custom.sceneToChange = (options[i] as any).controlScene;
          i = -1;
        }
      }
    ...
    element.notifications.add('setTransform', () => {
      ...
      const matrix = getTransform(t as any).matrix();
  ...
  arrow(...options: Array<OBJ_Arrow>) {
    ...
    element.custom.getFill = (border: Array<Point>, o: any) => [
      getArrowTris(border, o as any),
      'TRIANGLES',
    ];
    element.custom.updatePoints(joinObjects<any>({}, ...options));
    return element;
  ...
  grid(...optionsIn: Array<OBJ_Grid>) {
    ...
    const getTris = (points: any) => makePolyLine(
      points,
    ...
    element.custom.updatePoints = (updateOptions: Record<string, any>) => {
      const o = joinObjects<any>({}, element.custom.options, updateOptions);
      element.custom.options = o;
    ...
    element.custom.updatePoints(joinObjects<any>({}, ...optionsIn));
    return element;
  }
   ...
   */
  line(...options: Array<OBJ_Line>) {
    const element = this.polyline(joinObjects<any>(
      {},
      {
        transform: new Transform().scale(1).rotate(0).translate(),
      },
    ...
    const joinedOptions = joinObjects<any>({}, ...options);
    element.custom.options = joinObjects<any>(
      {},
      element.custom.options,
      {
        p1: [0, 0],
  ...
  textGL(options: Record<string, any>) {
    return Text(
      this.webgl[0],
      options as any,
    );
  ...
  */
  ...
  parseTextOptions(...optionsIn: Record<string, any>[]) {
    const defaultOptions = {
      text: '',
      font: {
        family: this.defaultFont.family,
  ...
  //   drawingObject: DrawingObject, optionsIn: Record<string, any>,
  // ) {
  ...
  html(optionsIn: {
    ...
    if (options.wrap || Array.isArray(options.element)) {
      ...
      if (Array.isArray(options.element)) {
        options.element.forEach((e: HTMLElement) => element.appendChild(e));
      } else {
