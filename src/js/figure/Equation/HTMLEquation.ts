...
class HTMLElementProperties {
  ...
  render(indent: number = 0, text: string = ''): string {
    return makeDiv(
      this.id,
      this.classes,
      text,
...
class HTMLElements extends HTMLElementProperties {
  ...
  constructor(
    ...
    super(id, classes);
    const filteredContent: Array<HTMLElement | HTMLElements | HTMLElementProperties> = [];
    content.forEach((c) => {
      if (c !== null) {
...
export default class HTMLEquation extends HTMLElements {
  ...
  createEq(content: Array<HTMLElements | HTMLElementProperties | string>) {
    const elements: Array<HTMLElement | HTMLElements | HTMLElementProperties> = [];
    content.forEach((c) => {
      if (typeof c === 'string') {
