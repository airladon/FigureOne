import { FigureElement, FigureElementCollection } from '../../Element';
import {
  Point, getPoint,
} from '../../../tools/g2';
import type {
...
export default class Box extends Symbol {
  ...
  surround(
    ...
    children: Array<string | FigureElement> | null | undefined,
    spaceIn: TypeParsablePoint | number = 0,
    drawingSpace: 'figure' | 'local' | 'gl' | 'draw' = 'local',
  ) {
    let elements: Array<FigureElement> = [parent];
    ...
    if (parent instanceof FigureElementCollection) {
      maxBounds = parent.getBoundingRect(drawingSpace, 'border', children as any);
    } else {
