...
type EQN_FormObjectDefinition = {
  ...
  duration?: number | null;               // null means to use velocity
...
export class Equation extends FigureElementCollection {
  ...
  eqn!: {
    ...
    formSeries: { [key: string]: Array<string> };
  ...
  constructor(
    shapes: Record<string, any>,
    options: EQN_Equation = {} as EQN_Equation,
  ) {
    let { color } = options;
    if (color == null) {
    ...
    if (options.font instanceof FigureFont) {
      ...
      optionsToUse.font = new FigureFont(
        joinObjects<any>({}, defaultFont, options.font),
      );
    } else {
    ...
    if (optionsToUse.formRestart != null
      ...
      optionsToUse.formRestart.pulse = joinObjects<any>({}, {
        scale: 1.1,
        duration: 1,
    ...
    this.fnMap.add('_goToForm', (percentage: number, customProperties: Record<string, any>) => {
      ...
      this.goToForm(joinObjects<any>({ ifAnimating: { cancelGoTo: false } }, customProperties, {
    ...
    this.fnMap.add('_showForm', (percentage: number, customProperties: Record<string, any>) => {
      this.showForm(customProperties.target);
    });
    (this.animations as any).goToForm = (...opt: Array<any>) => {
    ...
    this.animations.customSteps.push({
      step: (this.animations as any).goToForm.bind(this),
      name: 'goToForm',
    });

    this.fnMap.add('_nextForm', (percentage: number, customProperties: Record<string, any>) => {
      this.nextForm(joinObjects<any>({ ifAnimating: { cancelGoTo: false } }, customProperties, {
        callback: null,
      }));
      return this.getRemainingAnimationTime(['_Equation', '_EquationColor', '_EquationAnimateColor']);
    });
    (this.animations as any).nextForm = (...opt: Array<any>) => {
      const o = joinObjects<any>({}, {
        element: this,
        duration: 1,
      }, ...opt);
    ...
    this.animations.customSteps.push({
      step: (this.animations as any).nextForm.bind(this),
      name: 'nextForm',
    });
  }
  ...
  layoutForms(
    ...
    const arrange = (formName: string, setOnly: boolean, resetOnly: boolean) => {
  ...
  override contextLost() {
    ...
    if (this.eqn.functions.fullLineHeight != null) {
      this.eqn.functions.fullLineHeight.getAllElements().forEach(e => (e as any).contextLost());
    }
  }
  ...
  updateElementText(
    ...
    Object.keys(elements).forEach((elementName) => {
      ...
      (e as any).setText({ text: elements[elementName] });
  ...
  makeTextElem(
    ...
    if (options.font != null && options.font instanceof FigureFont) {
      ...
      fontDefinition = joinObjects<any>(
        {},
    ...
    if (
      options.style == null
      && (
        (options.font != null && (options.font as any).style == null)
        || options.font == null
      )
    ) {
      if (textToUse.match(/[A-Z,a-z,\u0370-\u03ff]/)) {
  ...
  getExistingOrAddSymbol(symbol: string | Record<string, any>) {
    if (typeof symbol === 'string') {
      return this.getExistingOrAddSymbolFromKey(symbol, {});
    }
    const [key, params] = Object.entries(symbol)[0];
    return this.getExistingOrAddSymbolFromKey(key, params as Record<string, any>);
  }
  ...
  // 'id_id_symbol'
  // 'symbol'
  getExistingOrAddSymbolFromKey(key: string, options: Record<string, any> = {}) {
    ...
    if (options.symbol != null && typeof options.symbol === 'string') {
      // debugger;
      const symbol = this.makeSymbolElem(options as any);
      if (symbol != null) {
        this.add(key, symbol);
        return symbol;
      }
  ...
  addElementFromKey(key: string, options: Record<string, any> = {}) {
    ...
    element = this.makeTextElem(joinObjects<any>({ text }, options));
  ...
  addElements(
    ...
    Object.keys(elems).forEach((key) => {
      ...
      if (typeof elem === 'string') {
        ...
        const { parent } = elem;
        if (parent != null) {
          delete (parent as any).elements[elem.name];
          (parent as any).drawOrder.splice((parent as any).drawOrder.indexOf(elem.name), 1);
          delete (parent as any)[`_${elem.name}`];
          elem.parent = null;
        }
        this.add(key, elem);
        ...
        let figureElem;
        if ((elem as any).symbol != null && typeof (elem as any).symbol === 'string') {
          figureElem = this.makeSymbolElem(elem as any);
        } else {
          figureElem = this.makeTextElem(elem as any, this.getTextFromKey(key));
        }
        if (figureElem != null) {
          if ((elem as any).mods != null) {
            figureElem.setProperties((elem as any).mods);
          }
  ...
  override setPosition(pointOrX: TypeParsablePoint | number, y: number = 0) {
  ...
  addForms(forms: EQN_Forms) {
    const isFormString = (form: any) => typeof form === 'string';
    const isFormArray = (form: any) => Array.isArray(form);
    const isFormMethodDefinition = (form: any) => {
    ...
    const isFormElements = (form: any) => (form instanceof Elements || form instanceof BaseAnnotationFunction);
    const addFormNormal = (name: string, form: TypeEquationForm) => {
      let formContent;
      try {
        formContent = [this.eqn.functions.contentToElement(form as any)];
      } catch (e: any) {
    ...
    const addFormFullObject = (name: string, form: TypeEquationForm) => {
      let formContent;
      try {
        formContent = [this.eqn.functions.contentToElement((form as any).content)];
      } catch (e: any) {
        throw new Error(`\n\nFigureOne error adding form '${name}' in equation '${this.getPath()}'\n\nError: '${e.message}'\n\nForm definition: ${JSON.stringify(form, null, 2)}\n\n`);
      }
      const {
  ...
  createForm(
    elements: any = this.elements,
  ) {
    return new EquationForm(
      elements,
      {
        getAllElements: this.getChildren.bind(this) as any,
        hideAll: this.hideAll.bind(this),
        show: this.show.bind(this),
        showOnly: this.showOnly.bind(this),
        stop: this.stopEquationAnimating.bind(this) as any,
  ...
  getPhraseElements(phrase: string | Array<string>) {
    ...
    const elementNames: Record<string, boolean> = {};
  ...
  getUnusedElements() {
    ...
    for (let j = 0; j < forms.length; j += 1) {
      const form = forms[j];
      const elems = this.getFormElements(form).map(e => (e as any).name);
  ...
  addForm(
    ...
    //   // its ok for this to start undefined, it will be filled.
    //   this.eqn.forms[name] = {};
    // }
    ...
    const transformElementMods = (elementMods: Record<string, any>) => {
    ...
    const transformTranslation = (trans: Record<string, any>) => {
    ...
    optionsToUse.alignment.fixTo = this.checkFixTo(optionsToUse.alignment.fixTo, elements as any);
    ...
    if (duration === 0) {
      ...
      if (options.animate === 'move') {
        ...
        this.animations.new('_Equation')
          ...
          .trigger({
            callback: 'showFormCallback' as any,
            duration: 0.01,
          })
        ...
        const newEnd = () => {
          ...
          if (pulse.element != null
            && pulse.element instanceof Equation
            && (pulse.element as Equation).getCurrentForm()!.name === form.name
          ) {
