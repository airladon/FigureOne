...
export function getFigureElement(
  elementsObject: { [key: string]: FigureElementPrimitive |
  ...
  if (elementsObject instanceof FigureElementCollection) {
    if (elementsObject && `_${name}` in elementsObject) {
      return (elementsObject as any)[`_${name}`];
    }
    return null;
  }
...
export class EquationFunctions {
  ...
  eqnMethod(name: string, params: any) {
    ...
    if (name === 'tBox') { return this.touchBox(params); }
    if (name === 'root') { return this.root(params); }
    if (name === 'brac') { return this.brac(params); }
    if (name === 'sub') { return this.sub(params); }
    if (name === 'sup') { return this.sup(params); }
    if (name === 'supSub') { return this.supSub(params); }
    if (name === 'topBar') { return this.topBar(params); }
    ...
    if (name === 'int') { return this.int(params); }
    if (name === 'sumOf') { return this.sumProd(params); }
    if (name === 'prodOf') { return this.sumProd(params); }
    if (name === 'matrix') { return this.matrix(params); }
    if (name === 'lines') { return this.lines(params); }
    ...
    try {
      ...
      if (Array.isArray(options)) {
        [
          ...
          minContentDescent, height, descent, fullContentBounds,
          useFullBounds,
        ] = options as any;
      } else {
        ({
  ...
  bar(
    options: EQN_Bar,
    forceOptions: Record<string, any> = {},
  ) {
    try {
      let content;
      let symbol;
      ...
      const o = joinObjects<any>({}, defaultOptions, optionsIn, forceOptions);
  ...
  annotate(options: EQN_Annotate) {
    ...
    const fillAnnotation = (ann: any) => {
      const annCopy = joinObjects<any>({}, defaultAnnotation, ann);
      annCopy.offset = getPoint(annCopy.offset);
      annCopy.content = this.contentToElement(ann.content);
      return annCopy;
    };

    const fillAnnotations = (anns: any) => {
    ...
    const glyphsToUse: Record<string, any> = {};

    const fillGlyphAnnotation = (side: string) => {
      if (glyphs == null) {
        return;
      }
      const glyphSide = (glyphs as any)[side];
      if (glyphSide == null) {
        return;
      }
      glyphsToUse[side] = {};
      ...
      glyphsToUse[side] = joinObjects<any>({}, defaultOptions[side], glyphSide);
  ...
  frac(
    ...
    try {
      ...
      return new Fraction(
        [this.contentToElement(numerator), this.contentToElement(denominator)],
        this.getExistingOrAddSymbol(symbol) as any,
        o,
      );
    } catch (e: any) {
  ...
  root(optionsOrArray: EQN_Root) {
    try {
      ...
      if (Array.isArray(optionsOrArray)) {
        [
          symbol, content, inSize,
          space, topSpace, rightSpace, bottomSpace, leftSpace,
          root, rootOffset, rootScale, fullContentBounds, useFullBounds,
        ] = optionsOrArray as any;
      } else {
        ({
          symbol, content, inSize,
          space, topSpace, rightSpace, bottomSpace, leftSpace,
  ...
  matrix(options: EQN_Matrix) {
    try {
      ...
      let contentArray = [];
      if (content != null) {
        contentArray = (content as any).map((c: any) => this.contentToElement(c));
      }
    ...
    try {
      ...
      if (content != null) {
        content.forEach((c: any) => {
          ...
          if (Array.isArray(c) || c.content == null) {
    ...
    try {
      ...
      if (Array.isArray(options)) {
        [
          ...
          limitsAroundContent,
          fromXPosition, fromYPosition, fromXAlign, fromYAlign,
          toXPosition, toYPosition, toXAlign, toYAlign,
          fullBoundsContent, useFullBounds,
        ] = options;
      } else {
        ({
          content, symbol, from, to, inSize, space,
          topSpace, bottomSpace,
          height, yOffset,
  ...
  processComment(
    ...
    if (Array.isArray(optionsOrArray)) {
      [
        ...
        scale, inSize, contentLineSpace, commentLineSpace,
  ...
  strike(
    ...
    try {
      ...
      const glyph = this.getExistingOrAddSymbol(symbol) as any;
  ...
  topStrike(...args: any[]) {
    try {
      ...
      ] = (this.processStrike as any)(...args);
      const annotations = [
        {
          content: comment,
          xPosition: 'center',
