...
function copyAnnotation(annotation: EQN_Annotation, namedCollection?: Record<string, any>) {
  return {
...
function copyAnnotations(
  annotations: Array<EQN_Annotation>,
  namedCollection?: Record<string, any>,
) {
  const copy: Array<EQN_Annotation> = [];
...
function copyGlyphs(
  ...
  Object.keys(glyphs).forEach((key) => {
    ...
    const copyGlyph: Record<string, any> = {};
...
function setPositionsForGlyphs(glyphs: EQN_Glyphs) {
  Object.keys(glyphs).forEach((key) => {
    ...
    t.updateScale([
      glyph.width == null ? 1 : glyph.width,
      glyph.height == null ? 1 : glyph.height,
    ]);
    t.updateTranslation([
...
export default class BaseAnnotationFunction implements ElementInterface {
  ...
  setLineGlyph(annotationBounds: Bounds, contentBounds: Bounds) {
    ...
    const pStart = getPositionInRect(
      contentBounds.toRect(), content.xAlign as any, content.yAlign as any,
    );
    const pEnd = getPositionInRect(
      annotationBounds.toRect(), annotation.xAlign as any, annotation.yAlign as any,
    );
  ...
  setAnnotationPosition(
    ...
    let xPos;
    let yPos;
    if ((contentToAnnotateBounds as any).annotations != null
      && annotation.reference != null) {
      const reference = (contentToAnnotateBounds as any).annotations[annotation.reference];
