...
class Element implements ElementInterface {
  ...
  calcSize(location: Point, scale: number) {
    ...
    if (content instanceof FigureElementCollection
      ...
      if (
        (content as any).drawingObject != null
        && (content as any).drawingObject.measureAndAlignText != null
      ) {
        (content as any).drawingObject.text[0].measureAndAlignText();
        (content as any).drawingObject.text[0].calcBorderAndBounds();
        (content as any).drawingObject.setBorder();
        (content as any).drawingObject.setTouchBorder();
        (content as any).custom.updateBorders({});
      }
...
class Elements implements ElementInterface {
  ...
  _dup(namedCollection?: Record<string, any>) {
    const contentCopy: Array<ElementInterface> = [];
  ...
  calcSize(location: Point, scale: number) {
    ...
    let fullBounds: Bounds | null = null;
  ...
  getAllElements(includeHidden: boolean = true) {
    let elements: Array<ElementInterface | FigureElementPrimitive | FigureElementCollection> = [];
    if (!includeHidden && !this.showContent) {
      return [];
    }
