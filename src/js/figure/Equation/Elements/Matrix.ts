...
function getMaxAscentDescent(
  ...
  const rowAscents: Array<number> = [];
  const rowDescents: Array<number> = [];
...
function getMaxColWidth(
  ...
  return [colWidths, maxWidth] as const;
}
...
export default class Matrix extends BaseEquationFunction {
  override calcSize(location: Point, scale: number) {
    ...
    const bounds: Array<Array<Bounds>> = [];
    const locs: Array<Array<Point>> = [];
    let index = 0;
