...
export default class SlideNavigator {
  ...
  equations!: { [key: string]: FigureElement };
  ...
  load(options: OBJ_SlideNavigator) {
    ...
    } else if (o.collection != null && (o.collection as any).elements != null) {
      this.collection = (o.collection as any).elements;
    }

    const processSlide = (payload: any) => {
    ...
    (this.collection as any).recorder.addEventType('slide', processSlide, true);
    const processAutoSlide = (payload: any) => {
    ...
    (this.collection as any).recorder.addEventType('_autoSlide', processAutoSlide, true);
    if ((this.collection as any).fnMap.global.map.slideNavigatorTransitionDone == null) {
      this.slideTransitionDoneName = 'slideNavigatorTransitionDone';
    } else {
      let index = 0;
      while ((this.collection as any).fnMap.global.map[`slideNavigatorTransitionDone${index}`] != null) {
        index += 1;
      }
      this.slideTransitionDoneName = `slideNavigatorTransitionDone${index}`;
    }
    (this.collection as any).fnMap.global.add(this.slideTransitionDoneName, this.transitionDone.bind(this));
    this.from = 'prev';

    this.doneCount = 0;
    (this.collection as any).fnMap.global.add('syncDone', this.syncDone.bind(this));

    if (o.slides != null) {
      this.slides = o.slides;
    }
    if (typeof o.text === 'string') {
      this.textElement = this.collection.getElement(o.text) as any;
    } else if (o.text != null) {
      this.textElement = o.text as any;
    } else {
      this.textElement = null;
    }
    let equations: Array<string | Equation> = [];
    ...
    this.equationDefaults = joinObjects<any>({}, {
      duration: 1,
      animate: 'move',
    }, o.equationDefaults || {});
    if (o.prevButton) {
    ...
    if (this.slides == null && this.equationsOrder.length === 1) {
      ...
      if (equation instanceof Equation) {
        const { eqn } = equation;
        if ((eqn as any).currentFormSeries != null && (eqn as any).currentFormSeries.length > 0) {
          this.slides = [];
          (eqn as any).currentFormSeries.forEach((f: any) => this.slides.push({ form: `${f}` }));
        }
      }
    }
    this.loadRecorder();
  ...
  syncDone = () => {
    this.doneCount -= 1;
    if (this.doneCount === 0) {
      (this.collection as any).fnMap.exec(this.slideTransitionDoneName);
    }
  }
  ...
  loadRecorder() {
    ...
    this.slides.forEach((slide, index) => {
      ...
      if (time != null || index === 0) {
        ...
        (this.collection as any).recorder.events._autoSlide.list.push([t, [index], 0]);
        lastTime = t;
      } else if (delta != null) {
        lastTime += delta;
        (this.collection as any).recorder.events._autoSlide.list.push([lastTime, [index], 0]);
      }
      if (execDelta != null && execDelta.length > 0) {
        let eDelta: any = execDelta;
        if (!Array.isArray(eDelta[0])) {
          eDelta = [eDelta];
        }
        eDelta.forEach((e: any) => {
          ...
          (this.collection as any).recorder.events._autoExec.list.push([t, [command], 0]);
        });
      }
    });
    ...
    (this.collection as any).recorder.events._autoExec.list.sort((a: any, b: any) => a[0] - b[0]);
  }
  ...
  setEquations(equationsIn: Array<string | Equation>) {
    const equations: { [key: string]: FigureElement } = {};
    const equationsOrder: Array<FigureElement> = [];
  ...
  getFormGeneric(formType: 'form' | 'fromForm', index: number) {
    ...
    if (formType === 'form') {
      ...
      form = (this.slides[index] as any)[formType];
    }
    if (form === undefined) {
      return {};
    }
  ...
  showForms(formsToShow: Record<string, any>) {
  ...
  setSteadyState(from: 'next' | 'prev' | number) {
    ...
    (this.collection as any).fnMap.exec(
      this.getProperty('steadyStateCommon', index, () => {}),
      from, index,
    );
    if (slide.steadyState != null) {
      (this.collection as any).fnMap.exec(slide.steadyState, from, index);
    }
    const { prevButton, nextButton } = this;
    if (prevButton != null) {
    ...
    if (nextButton != null && (nextButton as any).setLabel != null) {
      if (this.currentSlideIndex === this.slides.length - 1) {
        (nextButton as any).setLabel('Restart');
      } else {
        (nextButton as any).setLabel('Next');
      }
    }
    this.inTransition = false;
  ...
  processAutoTransitionAnim(
    ...
    if (step.trigger != null && key === 'trigger') {
      ...
      animSteps.push((this.collection.animations as any)[animName](o));
      ...
      animSteps.push(...elements.map(e => (e.animations as any)[animName](o)));
    }
  }
  ...
  processAutoTransitionSet(
    ...
    if (step[key] != null) {
      const elements = this.collection.getElements(step[key]);
      if (setKey !== '') {
        elements.map(e => (e as any)[setName](step[setKey]));
      } else {
        elements.map(e => (e as any)[setName]());
      }
    }
  }
  ...
  showAutoTransitionDissolve(index: number, showAll: boolean = false) {
    ...
    let stepsIn: any = slide.transition;
  ...
  autoTransition(stepsIn: Array<Array<Record<string, any>>> | Array<Record<string, any>>) {
    ...
    stepsIn.forEach((serialStep) => {
      ...
      const animSteps: Array<AnimationStep> = [];
      steps.forEach((step) => {
  ...
  transition(from: 'next' | 'prev' | number) {
    ...
    if (typeof slide.transition === 'function') {
      return (slide.transition as any)(this.slideTransitionDoneName, this.currentSlideIndex, from);
    }
    if (slide.transition != null && Array.isArray(slide.transition)) {
      return this.autoTransition(slide.transition as any);
    }
    if (slide.transition != null && typeof slide.transition === 'object') {
      return this.autoTransition([slide.transition] as any);
    }
    ...
    Object.keys(fromForms).forEach((eqnName) => {
      ...
      if (e != null && e instanceof Equation && forms[eqnName] !== undefined) {
        ...
        if (fromForm == null && toForm != null) {
          ...
          this.doneCount += 1;
          (e.animations.new() as any)
            .goToForm(joinObjects<any>({}, {
              start: fromForm,
              target: toForm,
            }, this.equationDefaults, {
              animate: (slide as any).animate || this.equationDefaults.animate,
            }))
            .whenFinished('syncDone')
            .start();
          done = null;
  ...
  setText(index: number) {
    ...
    (textElement as any).setText({
      text,
      modifiers: joinObjects<any>({}, commonMods, mods),
    });
  }
  ...
  showElements(index: number) {
    ...
    (this.collection as any).show(showCommon);
    (this.collection as any).show(show);
  ...
  hideElements(index: number) {
    ...
    (this.collection as any).hide(hideCommon);
    (this.collection as any).hide(hide);
  }
  ...
  goToSlide(slideIndex: number, from: 'next' | 'prev' | number | null = null) {
    ...
    if ((this.collection as any).recorder.state === 'recording') {
      if (fromToUse === 'next' || fromToUse === 'prev') {
        if (this.fromAutoSlide === false) {
          (this.collection as any).recorder.recordEvent('slide', [fromToUse, index]);
        }
      } else if (this.fromAutoSlide === false) {
        (this.collection as any).recorder.recordEvent('slide', ['goTo', index]);
      }
      this.fromAutoSlide = false;
    }
    ...
    (this.collection as any).setScenarios(this.getProperty('scenarioCommon', index, []));
    (this.collection as any).setScenarios(slide.scenario || []);
    (this.collection as any).fnMap.exec(
    ...
    if (slide.enterState != null) {
      (this.collection as any).fnMap.exec(slide.enterState, fromToUse, index);
    }
    if (slide.addReference && (this.collection as any).recorder.state === 'recording') {
      (this.collection as any).recorder.addCurrentStateAsReference();
    }
