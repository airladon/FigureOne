...
const transformBy = (inputTransforms: Array<Transform>, copyTransforms: Array<Transform>) => {
  const newTransforms: Array<Transform> = [];
  if (copyTransforms.length === 0) {
...
class FigureElement {
  ...
  touchScale: Point;
  isInteractive: boolean | null | undefined;         // Touch event is not processed by Figure
  ...
  setTransformCallback: string | ((t: Transform) => void) | null; // element.transform is updated
  ...
  constructor(
    ...
    const pulseTransformMethod = (mag: number, d: Point | null, type: string) => {
      if (type === 'scale') {
        if (d == null) {
      ...
      const x = mag * Math.cos(type as any);
      const y = mag * Math.sin(type as any);
  ...
  setProperties(properties: Record<string, any>, except: Array<string> | string = []) {
  ...
  _state(
    ...
    const o = joinObjects<any>({}, options, { returnF1Type: true });
    if (o.min) {
      state = getState(this, this._getStatePropertiesMin(), o as any);
    } else {
      state = getState(this, this._getStateProperties(o as any), o as any);
    }
    return state;
  }
  ...
  init(_webgl: any) {}
  _dup(_arg?: any): any { return this; }
  setupDraw(_now?: number, _canvasIndex?: number) {}
  draw(_now: number, _scene: Scene, ..._args: any[]) {}

  animateToState(
    state: Record<string, any>,
    options: Record<string, any>,
    independentOnly: boolean = false,
    startTime: number | 'now' | 'prevFrame' | 'nextFrame' | null = null,
  ) {
    ...
    if (Object.keys(target).length > 0) {
      const scenarioOptions = joinObjects<any>({}, options, { target });
    ...
    if (!this.arePulseTransformsSame(state, 0.001)) {
      ...
      let targetPulseTransforms = state.pulseTransforms.map((t: any) => getTransform(t));
      if (targetPulseTransforms.length === 0 && state.frozenPulseTransforms.length > 0) {
        targetPulseTransforms = state.frozenPulseTransforms.map((t: any) => getTransform(t));
      }
  ...
  dissolveInToState(
    ...
    this.frozenPulseTransforms = [];
    state.pulseTransforms.forEach((t: any) => this.frozenPulseTransforms.push(getTransform(t)));
    this.show();
    this.animations.new()
      .opacity({
  ...
  arePulseTransformsSame(state: Record<string, any>, delta: number = 0.001) {
  ...
  exec(
    execFunctionAndArgs: string | Array<any>,
  ) {
    ...
    if ((this as any)[execFunc] != null && typeof (this as any)[execFunc] === 'function') {
      if (args === undefined) {

        (this as any)[execFunc]();
      } else {

        (this as any)[execFunc](...args);
      }
    }
  }

  getElement(_elementPath?: any) {
    return this;
  }
   ...
   */
  setScale(scaleOrX: TypeParsablePoint | number, y: number | null = null, z: number = 1) {
    let scale;
  ...
  nextMovingFreelyFrame(now: number): void {
    ...
    if (this.state.isMovingFreely) {
      ...
      const next: Record<string, any> = this.decelerate(deltaTime);
  ...
  // retrieve a scenario
  getScenarioTarget(
    scenarioIn: string | OBJ_Scenario | null | undefined,
  ...
  setScenarios(scenarioName: string | Array<string>, _onlyIfVisible?: boolean) {
  ...
  decelerate(deltaTime: number | null = null): Record<string, any> {
    ...
    if (type === 'position' || type === 'translation') {
      next = decelerateVector(
        this.transform.t()!,
        this.state.movement.velocity as any,
        freely.deceleration as any,
        deltaTime,
        bounds as any,
        freely.bounceLoss as any,
        freely.zeroVelocityThreshold as any,
      );
      return {
    ...
    return decelerateValue(
      current,
      this.state.movement.velocity as number,
      freely.deceleration as number,
      deltaTime,
      bounds as any,
      freely.bounceLoss as number,
      freely.zeroVelocityThreshold as number,
    );
  }
  ...
  updateTransformWithMovement(
    ...
    const { type } = this.move;
    let value: number | Point = valueIn;
    if (this.move.bounds != null) {
      value = (this.move.bounds as any).clip(valueIn);
    }
    // console.log(type, transform)
    // let movement;
    if (type === 'scale') {
      transform.updateScale([value as number, value as number, value as number]);
    } else if (type === 'scaleX') {
      const s = transform.s()!;
      transform.updateScale([value as number, s.y, s.z]);
    } else if (type === 'scaleY') {
      const s = transform.s()!;
      transform.updateScale([s.x, value as number, s.z]);
    } else if (type === 'scaleZ') {
      const s = transform.s()!;
      transform.updateScale([s.x, s.y, value as number]);
    } else if (type === 'position' || type === 'translation') {
      transform.updateTranslation(value as any);
    } else if (type === 'rotation') {
      transform.updateRotation(value as number);
    }
    return transform;
  }
  ...
  moved(
    ...
    this.calcVelocity(previousValue as number | Point, value);
    const t = this.transform._dup();
  ...
  calcVelocity(
    ...
    if (typeof next === 'number' && typeof prevValue === 'number') {
      ...
      v = Math.min(Math.abs(v), this.move.maxVelocity as number);
      v *= d;
    } else {
      v = (next as Point).sub(prevValue as Point).scale(1 / deltaTime);
      const vMag = v.length();
      if (vMag <= (freely.zeroVelocityThreshold as number)) {
        v = new Point(0, 0, 0);
      } else if (vMag > (this.move.maxVelocity as number)) {
        v = v.normalize().scale(this.move.maxVelocity as number);
      }
    }
    // console.log(v)
  ...
  // Moving Freely
  startMovingFreely(callback: string | ((b: boolean) => void) | null = null): void {
    this.animations.cancelAll('freeze');
    this.stopBeingMoved();
    ...
    if (this.recorder.state === 'recording') {
      let v;
      if ((this.state.movement.velocity as any)._state) {
        v = (this.state.movement.velocity as any)._state();
      } else {
  ...
  stopMovingFreely(how: 'freeze' | 'cancel' | 'complete' | 'animateToComplete' | 'dissolveToComplete' = 'cancel'): void {
    ...
    if (how === 'complete' && wasMovingFreely) {
      ...
      const t = this.transform._dup();
      this.updateTransformWithMovement((result as any).value, t);
      // this.transformSet();
      this.setTransform(t);
    }
  ...
  pulse(optionsOrDone: null | OBJ_Pulse | (() => void) = null) {
    ...
    if (typeof optionsOrDone === 'function' || typeof optionsOrDone === 'string') {
      options = joinObjects<any>({}, defaultOptions);
      done = optionsOrDone;
    } else if (optionsOrDone == null) {
      options = joinObjects<any>({}, defaultOptions);
      done = null;
    } else {
      options = joinObjects<any>({}, defaultOptions, optionsOrDone);
      ({ done } = options);
    }
  ...
  stop(how: 'freeze' | 'cancel' | 'complete' | 'animateToComplete' | 'dissolveToComplete' = 'cancel', _elementOnly?: boolean) {
    let toComplete = 0;
    const checkStop = () => {
      toComplete -= 1;
      if (toComplete <= 0) {
  ...
  resize(figureHTMLElement: HTMLElement | null = null) {
  ...
  transformPoint(
    ...
    if (
      ...
      return this.pixelToPlane(p, toSpace, toSpacePlane) as Point;
    }

    // Moving from pixel space to gl space in 3D requires a plane
    // in figure space to cut through
    if (fromSpace === 'pixel' && toSpace === 'gl') {
      return this.pixelToGLPlane(p, toSpacePlane) as Point;
    }
  ...
  getCanvas() {
    if ((this as any).drawingObject != null) {
      return (this as any).drawingObject.getCanvas();
    }
  ...
  spaceTransformMatrix(
    ...
    const sceneOrNull = this.getScene();
    if (
      sceneOrNull == null
      && (
    ...
    const scene = sceneOrNull!;
    let figureToGLMatrix: Type3DMatrix = undefined as any;
    let figure2DSpace: { x: { min: number; span: number }; y: { min: number; span: number }; z: { min: number; span: number } } = undefined as any;
    if (scene != null) {
  ...
  */
  ...
  getBorder(
    ...
    const transformedBorders: Array<Array<Point>> = [];
    let matrix;
    if (Array.isArray(space)) {
      matrix = m3.mul(space as any, this.getTransform().matrix());
    } else {
  ...
  setTouchable(setOrOptions: OBJ_Touch | boolean = true) {
    ...
    // }
    if (this.parent != null) {
      (this.parent as any).setHasTouchableElements();
    }
  }
  ...
  setMove(options: boolean | OBJ_ElementMove) {
    ...
    if (bounds != null) {
      if ((bounds as any).contains != null) {
        this.move.bounds = bounds;
      } else if (
        (bounds as any).left !== undefined
        || (bounds as any).right !== undefined
        || (bounds as any).bottom !== undefined
        || (bounds as any).top !== undefined
        || (bounds as any).topDirection !== undefined
        || (bounds as any).rightDirection !== undefined
        || (bounds as any).position !== undefined
        || (bounds as any).normal !== undefined
      ) {
        const b = joinObjects<any>(
          {},
          {
        ...
        this.move.bounds = getBounds(bounds as any);
      }
    }
    if (maxVelocity != null) {
...
class FigureElementPrimitive extends FigureElement {
  ...
  override contextLost() {
    if ((this.drawingObject as any).contextLost != null) {
      (this.drawingObject as any).contextLost();
    }
  }
  ...
  override click(glPoint: Point = new Point(0, 0)) {
    super.click(glPoint);
    // if (this.drawingObject instanceof TextObjectBase) {
    //   let p = glPoint;
  ...
  override _dup(transform: Transform | null = null) {
    ...
    if (
      ...
      (primitive.pulseDefault as any).centerOn = (this.pulseDefault as any).centerOn;
    } else {
  ...
  override clear(canvasIndex: number = 0) {
    if ((this.drawingObject as any).clear != null) {
      (this.drawingObject as any).clear(canvasIndex, this.pulseTransforms);
    }
  }

  override resize(figureHTMLElement: HTMLElement | null | undefined = null) {
  ...
  override getBorderPoints(
    ...
    if (border === 'border') {
      ...
      if (isBuffer(this.border)) {
        return [getBoundingBorder(this.drawBorder, this.border as TypeParsableBuffer)];
      }
      return this.border as TypeBorder;
    }
    ...
    if (isBuffer(this.touchBorder)) {
      const b = this.getBorderPoints('border');
      return [getBoundingBorder(b, this.touchBorder as TypeParsableBuffer)];
    }
    return this.touchBorder as TypeBorder;
  }

  override setupDraw(now: number = 0) {
    if (this.isShown) {
      // let timer;
    ...
    if (this.isShown && (drawNumber == null || drawNumber === this.drawNumber)) {
      ...
      if (targetTexture) {
        if (parentUniqueColor != null) {
          ...
          colorToUse = this.uniqueColor!.map((c: number) => c / 255);
        } else {
          colorToUse = this.uniqueColor.map((c: number) => c / 255);
        }
      }
  ...
  override setFirstTransform(parentTransform: Transform = new Transform()) {
    // this.lastDrawElementTransformPosition = {
...
class FigureElementCollection extends FigureElement {
  ...
  constructor(options: OBJ_FigureElementCollection = {}) {
    ...
    if (options.touch != null) {
      this.setTouchable(options.touch as any);
    }
    ...
    if (o.border != null) {
      if (!isBuffer(o.border)) {
        this.border = getBorder(o.border as any) as any;
      } else {
    ...
    if (o.touchBorder != null) {
      if (!isBuffer(o.touchBorder)) {
        this.touchBorder = getBorder(o.touchBorder as any) as any;
      } else {
        this.touchBorder = o.touchBorder;
      }
    }
  ...
  override init(webgl: WebGLInstance) {
    this.getChildren().map((e: FigureElement) => e.init(webgl));
  }
  ...
  override _getStatePropertiesMin() {
    return [
  ...
  override getRootElement(): FigureElement | null {
    const e = super.getRootElement();
    if (e == null) {
      return this as FigureElement;
    }
    return e;
  }

  override _dup(exceptions: Array<string> = []) {
    ...
    if (
      ...
      (collection.pulseDefault as any).centerOn = (this.pulseDefault as any).centerOn;
    } else {
  ...
  toFront(elementsIn: Array<string | FigureElement> | string | FigureElement) {
    let elements: any = elementsIn;
    if (!Array.isArray(elementsIn) || typeof elementsIn === 'string') {
      elements = [elementsIn];
    }
    const names: Array<string> = [];
    elements.forEach((element: string | FigureElement) => {
    ...
    this.drawOrder.forEach((element: string) => {
  ...
  override isAnyElementMoving(): boolean {
    if (this.isShown === false) {
      return false;
    }
    if (this.isMoving()) {
  ...
  addElementWithName(
    ...
    (this as any)[`_${name}`] = this.elements[name];
    if (index !== -1) {
      this.drawOrder = [
    ...
    this.animateNextFrame();
    if ((element as any).isTouchable || (element as any).hasTouchableElements) {
  ...
  add(
    ...
    elementToAdd: FigureElement | null | undefined = null,
  ) {
    ...
    elements.forEach((elementDefinition, index) => {
      ...
      if (methodPath.slice(-1)[0].startsWith('add')) {
        ...
        if (elementModsToUse != null && (elementModsToUse as any) !== (({}) as any)) {
      ...
      if (
        `_${nameToUse}` in rootCollection
          && (addElementsToUse != null && (addElementsToUse as any) !== (({}) as any))
  ...
  getMethod(make: string) {
    const getPath = (e: Record<string, any>, remainingPath: Array<string>): any => {
      if (!(remainingPath[0] in e)) {
        return null;
      }
      if (remainingPath.length === 1) {
        return (e as any)[remainingPath[0]];
      }
      return getPath((e as any)[remainingPath[0]], remainingPath.slice(1));
    };
  ...
  override setFigure(figure: OBJ_FigureForElement, notify: boolean = true) {
  ...
  override setupDraw(
    ...
    if (this.isShown) {
      ...
      if (FIGURE1DEBUG) {
        ...
        const deltas = timer!.deltas();
        (window as any).figureOneDebug.setupDraw.push([
          '>>',
          this.getPath(),
          deltas[0],
          deltas.slice(1),
  ...
  override draw(
    ...
    if (this.isShown) {
      ...
      let parentTouchScaleToUse: Point | null = null;
      if (targetTexture) {
      ...
      let uniqueColorToUse: TypeColor | null = null;
      if (parentUniqueColor == null && this.isTouchable) {
        if (this.uniqueColor == null) {
          this.setUniqueColor(generateUniqueColor());
        }
        uniqueColorToUse = this.uniqueColor!.map((c: number) => c / 255);
  ...
  override exec(
    execFunctionAndArgs: string | Array<string | Record<string, any>>,
    elementsToExec: Array<string | FigureElement> | null | undefined = null,
  ) {
    ...
    elementsToExec.forEach((elementToExec) => {
      let element: FigureElement | null | undefined;
  ...
  override pulse(
    ...
    if (optionsOrElementsOrDone == null
      ...
      super.pulse(optionsOrElementsOrDone as any);
      return;
    }
    ...
    if (Array.isArray(optionsOrElementsOrDone)) {
      ...
      options = joinObjects<any>({}, optionsOrElementsOrDone);
      ({ elements } = options);
    ...
    elements.forEach((elementToPulse: string | FigureElement) => {
      let element: FigureElement | null | undefined;
   ...
   */
  override getElement(elementPath: (string | FigureElement) | null | undefined = null) {
    if (elementPath == null) {
      return this;
    }
    ...
    const getElement = (inputElementPath: string, parent: any): any => {
   ...
   */
  override getElements(children: TypeElementPath): Array<FigureElement> {
    // const paths = [];
    if (children == null) {
      return [];
    }
    const processPath = (path: any, prefix: string = ''): any => {
    ...
    const elements: Array<FigureElement> = [];
  ...
  override getShown() {
    ...
    const shown: Array<any> = [[this.getPath(), this.uid, this]];
    this.drawOrder.forEach((elementName: string) => {
  ...
  override getUid(uid: string): FigureElement | null {
    if (uid === this.uid) {
      return this as FigureElement;
    }
    for (let i = 0; i < this.drawOrder.length; i += 1) {
   ...
   */
  override show(toShow: TypeElementPath = []): void {
    super.show();
    this.getElements(toShow).forEach((element: FigureElement) => {
      element.showAll();
    });
  }
  ...
  override hide(
    ...
    this.getElements(toHide).forEach((element: FigureElement) => {
      element.hide();
    });
  }
  ...
  resizeHtmlObject() {
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      (element as any).resizeHtmlObject();
    }
  }

  override resize(figureHTMLElement: HTMLElement | null = null) {
    super.resize(figureHTMLElement);
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
  ...
  override getBorderPoints(
    ...
    const getBorderFromChildren = (b: string) => {
      const childrenBorder: Array<any> = [];
      let childrenToUse: Array<any> | null = children;
      ...
      childrenToUse.forEach((child: any) => {
        ...
        if (e instanceof FigureElementCollection || (e as any).drawBorder != null) {
          childrenBorder.push(...e.getBorder('local', b as any, null, shownOnly));
        }
      });
    ...
    if (border === 'border') {
      ...
      if (isBuffer(this.border)) {
        return [getBoundingBorder(getBorderFromChildren('border'), this.border as any)];
      }
      return this.border as any;
    }
    ...
    if (isBuffer(this.touchBorder)) {
      return [getBoundingBorder(getBorderFromChildren('touchBorder'), this.touchBorder as any)];
    }
    return this.touchBorder as any;
    // }
  }
  ...
  override getBorder(
    ...
    children: Array<string | FigureElement> | null | undefined = null,
    shownOnly: boolean = true,
  ) {
    ...
    return borderPoints.map((b: any) => b.map((p: any) => getPoint(p).transformBy(matrix)));
  }

  override getBoundingRect(
    space: TypeCoordinateSpace = 'local',
    border: 'touchBorder' | 'border' = 'border',
    children: Array<string | FigureElement> | null | undefined = null,
    shownOnly: boolean = true,
    ...
    return getBoundingRect(transformedBorder as any);
  }

  override getRelativeBoundingRect(
    space: TypeCoordinateSpace = 'local',
    border: 'border' | 'touchBorder' = 'border',
    children: Array<string | FigureElement> | null | undefined = null,
    shownOnly: boolean = true,
  ...
  override setUniqueColor(color: null | TypeColor) {
  ...
  override getUniqueColorElement(color: TypeColor): FigureElement | null {
  ...
  override getTouched(glLocation: Point): Array<FigureElement> {
    if (!this.isTouchable && !this.hasTouchableElements) {
      return [];
    }
    let touched: Array<FigureElement> = [];
    if (this.isTouchable) {
  ...
  override getSelectionFromBorders(glLocation: Point) {
    if (!this.isTouchable && !this.hasTouchableElements) {
      return null;
    }
    if (this.isTouchable) {
  ...
  setFont(fontSize: number) {
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      (element as any).setFont(fontSize);
    }
  }
  ...
  override setDimColor(color: TypeColor = [0, 0, 0, 1]) {
    const nonNullColor = color != null ? color : [0, 0, 0, 0];
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
  ...
  override undim(children: TypeElementPath | null | undefined = undefined) {
    let elements;
    if (children != null) {
    ...
    elements.forEach((element: FigureElement) => element.undim());
  }

  override dim(children: TypeElementPath | null | undefined = undefined) {
    let elements;
    if (children != null) {
      elements = this.getElements(children);
    } else {
    ...
    elements.forEach((element: FigureElement) => element.dim());
  }

  override highlight(elementsToHighlight: TypeElementPath | null | undefined = null) {
  ...
  setElementTransforms(elementTransforms: Record<string, any>) {
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
  ...
  reorder() {
    this.drawOrder.sort((a: string, b: string) => {
  ...
  animateToTransforms(
    ...
    name: string = '',
    easeFunction: string | ((n: number) => number) = 'tools.math.easeinout',
  ) {
   ...
   */
  getAllPrimitives() {
    let elements: Array<FigureElement> = [];
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      if (element instanceof FigureElementCollection) {
   ...
   */
  getAllElements() {
    const elements: Array<FigureElement> = [this as any];
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      // elements.push(element);
      if (element instanceof FigureElementCollection) {
  ...
  recreateAtlases() {
    ...
    Object.keys(this.textureAtlases).forEach(
      (id: string) => this.textureAtlases[id].atlas.recreate(),
    );
  }
  ...
  override getAllElementsWithScenario(scenario: string): Array<FigureElement> {
    let elements: Array<FigureElement> = super.getAllElementsWithScenario(scenario) as Array<FigureElement>;
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
  ...
  override getNextAnimationFinishTime() {
    if (this.simple) {
      if (
        this.state.isMovingFreely || this.state.isChanging || this.animations.state === 'animating'
      ) {
    ...
    let minRemainingTime: number | null = null;
  ...
  override getRemainingAnimationTime(
    animationNames: Array<string> | string = [],
  ): number {
    const elements = this.getAllElements();
  ...
  goToStep(step: number) {
    ...
    elems.forEach((e: Element, index: number) => {
      if (index === step) {
  ...
  setupWebGLBuffers(newWebgl: WebGLInstance) {
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      (element as any).setupWebGLBuffers(newWebgl);
    }
  }
  ...
  changeWebGLInstance(newWebgl: WebGLInstance) {
    ...
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      oldInstance = (element as any).changeWebGLInstance(newWebgl);
    }
    return oldInstance;
  }
  ...
  getLoadingElements() {
    ...
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      ...
      if (element instanceof FigureElementPrimitive) {
        ...
        elems = [...elems, ...(element as any).getLoadingElements()];
      }
    }
    return elems;
  }
  ...
  setPointsFromDefinition() {
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      const element = this.elements[this.drawOrder[i]];
      if ((element as any).setPointsFromDefinition != null) {
        (element as any).setPointsFromDefinition();
      }
    }
  }
  ...
  setPrimitiveColors() {
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      ...
      if (element instanceof FigureElementPrimitive) {
        ...
        (element as any).setPrimitiveColors();
      }
    }
  }
  ...
  unrenderAll() {
    ...
    for (let i = 0; i < this.drawOrder.length; i += 1) {
      ...
      if (element instanceof FigureElementPrimitive) {
        ...
        (element as any).unrenderAll();
      }
    }
  }


  override setScenarios(
  ...
  override saveScenarios(scenarioName: string, keys: Array<string>) {
  ...
  override animateToState(
    state: Record<string, any>,
    options: Record<string, any>,
    independentOnly: boolean = false,
    startTime: number | 'now' | 'prevFrame' | 'nextFrame' | null | undefined = null,
  ) {
    let duration = 0;
  ...
  override setTimeDelta(delta: number) {
  ...
  override clearFrozenPulseTransforms() {
  ...
  override freezePulseTransforms(forceOverwrite: boolean = true) {
  ...
  override _state(
    ...
    const tempOptions = joinObjects<any>({}, options, { returnF1Type: true });
    if (tempOptions.min) {
