...
class Figure {
  ...
  constructor(options: OBJ_Figure = {}) {
    ...
    const optionsToUse = joinObjects<any>({}, defaultOptions, options);
    const {
      htmlId,
    } = optionsToUse;
      ...
      if (container instanceof HTMLElement) {
        ...
        for (let i = 0; i < children.length; i += 1) {
          ...
          if (child.classList.contains('figureone__html')
            ...
            this.htmlCanvas = child as HTMLElement;
          }
        }
  ...
  addSlideNavigator(options: COL_SlideNavigator) {
    const nav = this.add(joinObjects<any>(
      {},
      {
        name: '_nav_',
   ...
   */
  addCursor(options: Record<string, any>) {
    const cursor = this.add(joinObjects<any>(
      {},
      {
        name: '_cursor_',
  ...
  bindRecorder() {
    ...
    const onAutoCursor = (payload: any) => {
      const [action, x, y] = payload;
      if (action === 'show') {
        this.showCursor('up', new Point(x, y));
      } else {
    ...
    const onAutoCursorMove = (payload: any) => {
      const [x, y] = payload;
      // this.setCursorGLPoint(new Point(x, y));
      this.setCursor(new Point(x, y));
  ...
  getState(options: { precision?: number, ignoreShown?: boolean, min?: boolean }) {
    ...
    this.stateTime = this.timeKeeper.now() / 1000;
    const o = joinObjects<any>({}, options, { returnF1Type: false });
  ...
  setState(
    ...
    const state = parseState(stateIn, this as any);
    ...
    if ((optionsIn as any).velocity != null) {
      options.velocity = {
        position: 2,
        rotation: Math.PI * 2 / 2,
        scale: 1,
    ...
    if (typeof optionsIn === 'string') {
      ...
      options = joinObjects<any>({}, options, optionsIn);
    ...
    if (options.how === 'dissolve') {
      ...
      if (options.duration === 0) {
        ...
        options.duration = joinObjects<any>({}, defaultDuration, options.duration);
      }
    ...
    if (
      ...
      this.dissolveToState({
        state,
        dissolveInDuration: (options.duration as any).dissolveIn,
        dissolveOutDuration: (options.duration as any).dissolveOut,
        done: finished,
        delay: (options.duration as any).delay,
        startTime: 'now',
      });
    }
  ...
  */
  ...
  getRemainingAnimationTime(nowIn: number = this.timeKeeper.now() / 1000) {
    ...
    for (let i = 0; i < elements.length; i += 1) {
      ...
      const remainingMovingFreelyTime = (element as any).getRemainingMovingFreelyTime(now);
  ...
  setAnimationFinishedCallback(callback: string | (() => void) | null | undefined) {
  ...
  animationFinished() {
    ...
    this.fnMap.exec(this.animationFinishedCallback as any);
  ...
  renderElementToTiedCanvas(elementName: string) {
    ...
    const htmlCanvas = document.getElementById(elementToRender.tieToHTML.element as string);
    ...
    this.renderToCanvas(elementToRender.tieToHTML.element as string);
  ...
  renderToCanvas(
    ...
    let htmlCanvas: string | HTMLElement | null = htmlCanvasElementOrId;
    ...
    if (w instanceof HTMLImageElement) {
      w.src = (this.webglOffscreen.gl.canvas as HTMLCanvasElement).toDataURL('image/png', 0.5);
    ...
    if (d instanceof HTMLImageElement) {
      d.src = (this.draw2DOffscreen.canvas as HTMLCanvasElement).toDataURL('image/png', 0.5);
  ...
  showCursor(show: 'up' | 'down' | 'hide', position: Point | null | undefined = null) {
  ...
  //           this.rotateElement(  //             elementToMove,
  //             previousFigurePoint,
  //             currentFigurePoint,
  //           );
  //         } else if (element.move.type === 'scale') {
  //           this.scaleElement(  //             elementToMove,
  //             previousFigurePoint,
  //             currentFigurePoint,
  //           );
  ...
  //           );
  //         } else {
  //           this.translateElement(  //             elementToMove,
  //             previousFigurePoint,
  //             currentFigurePoint,
  //           );
  //         }
  ...
  setupAnimations() {
    this.elements.fnMap.add('_cameraCallback', (p: number, customProperties: Record<string, any>) => {
    ...
    (this.animations as any).camera = (...opt: Array<any>) => {
      const o = joinObjects<any>({}, {
        progression: 'easeinout',
      }, ...opt);
      o.customProperties = {
        start: joinObjects<any>({}, this.scene.camera, o.start || {}),
        target: joinObjects<any>({}, this.scene.camera, o.target || {}),
      };
    ...
    this.animations.customSteps.push({
      step: (this.animations as any).camera.bind(this),
      name: 'camera',
    });
  }
  ...
  createFigureElements() {
    this.elements = this.collections.collection({ name: 'rootCollection' } as any);
    // this.elements.scene = this.scene;
    // this.animations = this.elements.animations;
    // this.elements.getCanvas = () => this.canvasLow;
    // this.setupAnimations();
  ...
  getSelectionFromPixel(xPixel: number, yPixel: number, debug: boolean = false) {
    ...
    const x = xPixel / (gl.canvas as HTMLCanvasElement).clientWidth * gl.canvas.width;
    const y = gl.canvas.height - yPixel * gl.canvas.height / (gl.canvas as HTMLCanvasElement).clientHeight - 1;
    ...
    return this.elements.getUniqueColorElement(data as any);
  }
  ...
  showBorders(
    ...
    if (element == null || element === 'touchable') {
      ...
      elements = this.getElements(element) as any;
    } else {
      elements = [element];
    }
    let colorIndex = 0;
  ...
  pause() {
    if ((this.state.pause as string) === 'pause' || this.state.pause === 'preparingToPause' || this.state.pause === 'preparingToUnpause') {
      return;
    }
  ...
  draw(nowIn: number, canvasIndex: number = 0): void {
    ...
    if ((this.elements as any).__frameRate_ != null || FIGURE1DEBUG) {
      timer = new PerformanceTimer();
      (window as any).figureOneDebug.draw = [];
    ...
    if (this.scrolled === true) {
      ...
      if (Math.abs(window.pageYOffset - this.oldScroll)
          > (this.webglLow.gl.canvas as HTMLCanvasElement).clientHeight / 4
      ) {
        this.renderAllElementsToTiedCanvases();
        // }
    ...
    this.drawQueued = false;
    this.flushMoveBuffer();
    if ((this.elements as any).__frameRate_ != null || FIGURE1DEBUG) { timer!.stamp('m1'); }

    this.notifications.publish('beforeDraw');
    if ((this.elements as any).__frameRate_ != null || FIGURE1DEBUG) { timer!.stamp('beforeDraw'); }
    if ((this.elements as any).__frameRate_ != null && this.frameRate.information != null) {
        (this.elements as any).__frameRate_.setText({ text: this.frameRate.information, reform: false });
    }
    this.elements.setupDraw(
      now,
      canvasIndex,
    );
    if ((this.elements as any).__frameRate_ != null || FIGURE1DEBUG) { timer!.stamp('setupDraw'); }

    this.clearContext(canvasIndex);
    if ((this.elements as any).__frameRate_ != null || FIGURE1DEBUG) { timer!.stamp('clearContext'); }
    this.elements.draw(
      now,
      this.scene,
      [new Transform()],
    ...
    if ((this.elements as any).__frameRate_ != null || FIGURE1DEBUG) { timer!.stamp('draw'); }
    ...
    if (FIGURE1DEBUG || (this.elements as any).__frameRate_ != null) {
      timer!.stamp('afterDraw');
      const deltas = timer!.deltas();
      if (FIGURE1DEBUG) {
        if ((window as any).figureOneDebug.cumTimes.length > 50) {
          Console(
            '>>>>>>>>>>> Total',
            round(
              (window as any).figureOneDebug.cumTimes.reduce((sum: number, time: number) => sum + time) / 50,
              2,
            ),
            { frameTotal: deltas[0] },
            { frame: deltas.slice(1) },
            { setupDraw: (window as any).figureOneDebug.setupDraw },
            { draw: (window as any).figureOneDebug.draw },
            { misc: (window as any).figureOneDebug.misc },
          );
          (window as any).figureOneDebug.cumTimes = [];
        } else {
          (window as any).figureOneDebug.cumTimes.push(deltas[0]);
        }
        // }
        (window as any).figureOneDebug.history.push({
          ...
          draw: (window as any).figureOneDebug.draw,
          misc: (window as any).figureOneDebug.misc,
          animationManager: (window as any).figureOneDebug.animationManager,
        });
      }
        if ((this.elements as any).__frameRate_ != null) {
        const timeBetweenFrames =
          (this.timeKeeper.now() - ((this.timeKeeper as any).lastDrawTime || 0)) / 1000;
  ...
  centerDrawingLens(fromTimeOut: boolean = false) {
    ...
    let newTop = window.pageYOffset + viewPortHeight / 2
                 - (this.webglLow.gl.canvas as HTMLCanvasElement).clientHeight / 2;
    if (newTop < 0) {
      newTop = 0;
    }
    const newTopInPx = `${newTop}px`;
    if ((this.webglLow.gl.canvas as HTMLCanvasElement).style.top !== newTopInPx) {
      (this.webglLow.gl.canvas as HTMLCanvasElement).style.top = `${newTop}px`;
      (this.draw2DLow.canvas as HTMLCanvasElement).style.top = `${newTop}px`;
  ...
  animateNextFrame(draw: boolean = true, fromWhere: string = '') {
    ...
    if (!this.drawQueued) {
      ...
      this.timeKeeper.queueNextFrame(this.draw.bind(this) as any);
    }
  }
  ...
  debugShowTouchBorders(
    ...
    elems.forEach((element, index) => {
      ...
      const polyline = this.primitives.polyline(joinObjects<any>({}, lineOptions, {
  ...
  addFrameRate(numFrames: number = 10, options: OBJ_FormattedText = {}) {
    ...
    const frame = this.add(joinObjects<any>(
      {},
      {
        name: '_frameRate_',
  ...
  addZoomPanControl(options: {
    ...
    const control = this.add(joinObjects<any>(
      {},
      {
        name: '_zoomPan_',
