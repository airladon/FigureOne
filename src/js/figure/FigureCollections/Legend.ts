...
class CollectionsPlotLegend extends FigureElementCollection {
  ...
  getTracesToShow(o: { show?: any; hide?: any }) {
  ...
  getOffset(o: { offset: Array<TypeParsablePoint> | TypeParsablePoint }) {
    ...
    for (let i = 0; i < this.toShow.length; i += 1) {
      offsetArray.push((offset[i % offset.length] as any)._dup());
    }
    return offsetArray;
  }
  ...
  addTraces(o: COL_PlotLegend) {
    ...
    this.toShow.forEach((traceIndex, index) => {
      ...
      let custom: any = {};
      if ((o.custom as any)[`${index}`] != null) {
        custom = (o.custom as any)[`${index}`];
      } else if ((o.custom as any)[trace.name] != null) {
        custom = (o.custom as any)[trace.name];
      }
      ...
      if (custom.font != null) {
        textOptions.font = joinObjects<any>({}, textOptions.font, custom.font);
      }
      ...
      let colorOverride: Record<string, any> = {};
      if (o.fontColorIsLineColor) {
        colorOverride = { font: { color: trace.color.slice() } };
      }
      const oText = joinObjects<any>({}, textOptions, colorOverride, textOptionsToUse, custom.text);
      // o.offset = getPoint(o.offset);
      const traceName = this.collections.text(oText);
      this.add(`trace${traceIndex}`, traceName as any);
    });
  }

  addFrame(optionsIn: Record<string, any> | boolean | Array<number>) {
    let o: any = optionsIn;
    if (optionsIn === true) {
      o = {};
    ...
    const defaultFrameOptions: Record<string, any> = {
      // space: Math.min(bounds.width, bounds.height) / 20,
      space: this.defaultFont.size,
      // line: { width: 0.01 },
    };
    if (o.line == null && o.fill == null) {
      defaultFrameOptions.line = { width: 0.01 };
    }

    const oFrame = joinObjects<any>({}, defaultFrameOptions, o);
  ...
  update() {
    ...
    elements = elements.filter(e => (e as any).parent.name !== 'frame');
    if (this._frame != null) {
      this._frame.surround(elements, this.frameSpace, false);
    }
  }

  // _getStateProperties(options: Record<string, any>) {  // eslint-disable-line class-methods-use-this
