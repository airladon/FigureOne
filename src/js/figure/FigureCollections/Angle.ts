...
export type TypeAngleLabelOptions = {
  ...
  autoHide?: number | null | undefined;             // Auto hide label if angle is less than this
  autoHideMax?: number | null | undefined;          // Auto hide label if angle greater than this
  update?: boolean;
...
export type OBJ_AngleCurve = {
  ...
  autoHide?: number | null | undefined;       // if angle is less than this, hide curve
  autoHideMax?: number | null | undefined;    // if angle is less than this, hide curve
...
class AngleLabel extends EquationLabel {
  ...
  constructor(
    ...
    precision: number = 0,
    autoHide: number | null | undefined = null,
    autoHideMax: number | null | undefined = null,
...
class CollectionsAngle extends FigureElementCollection {
  ...
  side1: { width: number; length: number } | null | undefined;
  side2: { width: number; length: number } | null | undefined;
  curve: any;
  ...
  override collections!: FigureCollections;
  ...
  constructor(
    ...
    options: COL_Angle = {} as COL_Angle,
  ) {
    const defaultOptions = {
    ...
    if (optionsToUse.arrow != null) {
      ...
      this.arrow = arrowOptions as any;
      if (this.arrow == null) {
        return;
      }
      ...
      if (this.arrow.start != null) {
        (this.arrow as any).start = joinObjects<any>({}, defaultO, this.arrow.start);
      }
      if (this.arrow.end != null) {
        (this.arrow as any).end = joinObjects<any>({}, defaultO, this.arrow.end);
      }
      this.addArrow('start');
      this.addArrow('end');
    }
    ...
    if (optionsToUse.sides) {
      ...
      const sideOptions = joinObjects<any>({}, defaultSideOptions, sides);
      this.addSide(1, sideOptions.length, sideOptions.width, sideOptions.color);
      this.addSide(2, sideOptions.length, sideOptions.width, sideOptions.color);
    }
    ...
    this.animations.angle = (...opt: Array<any>) => {
      ...
      o.callback = (percentage: number) => {
        ...
        const a = ((target as number) - (start as number)) * percentage + (start as number);
    ...
    this.animations.pulseAngle = (...opt: Array<any>) => {
      const o = joinObjects<any>({}, {
        element: this,
        duration: 1,
      }, ...opt);
    ...
    this.update();
    if (optionsToUse.mods != null) {
      this.setProperties(optionsToUse.mods);
    }
  }

  override _getStateProperties(options: Record<string, any>) {  // eslint-disable-line class-methods-use-this
  ...
  _fromState(state: Record<string, any>) {
  ...
  setAngle(options: OBJ_AngleSet = {}) {
    ...
    if (corner != null && _corner != null) {
      ...
      (_corner as any).custom.updatePoints({ points });
    }
    this.update();
    if (wasHidden) {
      this.hide();
  ...
  addSide(
    ...
    (this as any)[`side${index}`] = { length, width };
    this.add(`side${index}`, line as any);
  }

  addLabel(options: TypeAngleLabelOptions = {} as TypeAngleLabelOptions) {
    const defaultLabelOptions: Record<string, any> = {
      text: null,
      radius: 0.4,
      curvePosition: 0.5,
      offset: 0.0001,
      ...
      location: 'outside',
      update: false,
      font: joinObjects<any>({}, this.collections.primitives.defaultFont),
    };
    ...
    const optionsToUse = joinObjects<any>({}, defaultLabelOptions, options);
    ...
    if (this.label != null) {
      this.add('label', this.label.eqn as any);
    }
  ...
  addCurve(curveOptions: {
    ...
    if (this.curve != null && this.curve.autoRightAngle) {
      ...
      this.add('curveRight', right as any);
    }
  }
  ...
  change(options: {
    ...
    if (this._curve != null && options.radius != null) {
      (this._curve as any).custom.update({ radius: options.radius });
    }
    if (this.label != null) {
  ...
  addArrow(
    ...
    if ((this.arrow as any)[lineEnd] == null) {
      return;
    }
    const o = (this.arrow as any)[lineEnd];
    const a = this.collections.primitives.arrow(joinObjects<any>(
      {},
      o,
      {
        // angle: r,
    ...
    const arrowLength = (getArrowLength as any)(o)[1];
    let index = 1;
    if (lineEnd === 'end') {
      index = 2;
    }
    (this.arrow as any)[lineEnd] = {
      height: arrowLength,
      radius: o.radius || 0,
      autoHide: o.autoHide,
      curveOverlap: o.curveOverlap,
    };
    this.add(`arrow${index}`, a as any);
    this.update();
  }
  ...
  updateCurve(primaryCurveAngle: number, angle: number, rotation: number, show: boolean) {
    ...
    if (curve) {
      ...
      for (let i = 0; i < curve.num; i += 1) {
        ...
        const element = (this as any)[name];
        if (element) {
          if (show) {
            element.show();
            if (i === 0) {
  ...
  update(labelRotationOffset: number | null = null) {
    ...
    if (_arrow1 && this.arrow != null && this.arrow.start != null) {
      if (arrow1Hide) {
        ...
        _arrow1.show();
        _arrow1.transform.updateTranslation([this.arrow.start.radius, 0] as any);
    ...
    if (_arrow2 && this.arrow != null && this.arrow.end != null) {
      if (arrow2Hide) {
        ...
        let curveToLine;
        const radius = (this.arrow.start as any).radius || 0;
        if (
          (this.angle > 0 && this.direction === 1)
          || (this.angle > 0 && this.direction === -1)
        ) {
    ...
    if (_side1 && side1) {
      ...
      _side1.transform.updateScale([(side1 as any).length, 1]);
    }

    const { _side2, side2 } = this;
    if (_side2 && side2) {
      _side2.transform.updateRotation(this.angle);
      _side2.transform.updateScale([(side2 as any).length, 1]);
    }
   ...
   */
  updateLabel(rotationOffset: number | null | undefined = this.getRotation(), forceShow: boolean = true) {
    ...
    if (_label && label) {
      if (
        ...
        let orientation: any = label.orientation;
        let location: any = label.location;
  ...
  pulseAngle(options?: OBJ_PulseAngle) {
    ...
    const o = joinObjects<any>(defaultOptions, o_options);
    let { done } = o;
    const pulse = (elementName: string, oName: string, oScale: any, oThick: any, centerOn: any = null) => {
      const element = this.elements[elementName];
      if (element != null && element.isShown) {
        let pulseOptions;
        const defaultThick = joinObjects<any>({ num: o.thick }, o.thick > 1 ? oThick : oScale);
        if (typeof o[oName] === 'number') {
          pulseOptions = joinObjects<any>(
   ...
   */
  override setMovable(movableOrOptions: boolean | OBJ_MovableAngle = true) {
    const defaultOptions = {
      movable: true,
      type: this.move.type,
      width: 0.5,
    ...
    if (movableOrOptions === false) {
      options = joinObjects<any>({}, defaultOptions, { movable: false });
    } else if (movableOrOptions === true) {
      options = defaultOptions;
    } else {
      options = joinObjects<any>({}, defaultOptions, movableOrOptions);
    }
    const {
  ...
  addAnglePad(percentLength: number = 1, width: number = 0.5, type: 'rotation' | 'angle') {
    if (this._anglePad == null) {
      ...
      this.add('anglePad', anglePad as any);
      anglePad.setMovable();
      anglePad.move.type = 'rotation';
      anglePad.drawingObject.border = [[]];
      if (type === 'rotation') {
        // anglePad.move.element = this;
        anglePad.notifications.add('beforeSetTransform', (transformToSet: any) => {
  ...
  addRotPad(percentLength: number = 1, width: number = 0.5, type: 'rotation' | 'angle') {
    if (this._rotPad == null) {
      ...
      if (type === 'angle') {
        ...
        this.notifications.add('beforeSetTransform', ((transformToSet: any) => {
          const nextR = transformToSet[0].r();
  ...
  addMovePad(percentLength: number = 1) {
    if (this._movePad == null) {
      ...
      this.add('movePad', movePad as any);
      movePad.setMovable();
