...
function processArray(
  toProcess: Record<string, any> | Array<Record<string, any>>,
  defaultOptions: Record<string, any>,
  defaultLabelOptions: Record<string, any>,
  total: number,
) {
  ...
  if (toProcessDefaults.hide != null) {
    toProcessDefaults.hide.forEach((index: number) => {
  ...
  for (let i = 0; i < total; i += 1) {
    if (show.indexOf(i) === -1) {
      ...
      let indexOptions = {};
      if ((toProcess as any)[`${index}`] != null) {
        indexOptions = (toProcess as any)[`${index}`];
      }
      let labelDefaults = {};
      if (labels.length > 0) {
      ...
      const o = joinObjects<any>({}, defaultOptions, toProcessDefaults, labelDefaults, indexOptions);
      if (o.label != null) {
...
export default class CollectionsPolyline extends FigureElementCollection {
  ...
  override animateNextFrame!: () => void;
  ...
  constructor(
    ...
    options: COL_Polyline = {} as COL_Polyline,
  ) {
    ...
    delete defaultOptions.font.style;
    ...
    this.points = optionsToUse.points.map((p: any) => getPoint(p));
  ...
  addPads(pad: Record<string, any>) {
    ...
    for (let i = 0; i < pCount; i += 1) {
      if (padArray[i] != null) {
        const name = `pad${i}`;
        const padOptions = joinObjects<any>({}, {
        ...
        this.add(name, padShape as any);
      }
    }
  }

  addAngles(angle: Record<string, any>, close: boolean) {
    ...
    for (let i = 0; i < pCount; i += 1) {
      ...
      if (angleArray[i] != null) {
        ...
        const angleOptions = joinObjects<any>({}, {
          p1: this.points[p1],
          p2: this.points[p2],
          p3: this.points[p3],
        }, angleArray[i]);
        const angleAnnotation = this.collections.angle(angleOptions);
        this.add(name, angleAnnotation as any);
      }
    }
  }


  addSides(side: Record<string, any>, close: boolean) {
    ...
    for (let i = 0; i < pCount; i += 1) {
      ...
      if (sideArray[i] != null) {
        const name = `side${i}${j}`;
        let sideOptions = joinObjects<any>({}, {
          p1: this.points[i],
          p2: this.points[j],
        }, sideArray[i]);
        if (this.reverse) {
          sideOptions = joinObjects<any>({}, {
            p1: this.points[j],
            p2: this.points[i],
          }, sideArray[i]);
        }
        const sideLine = this.collections.line(sideOptions);
        (sideLine as any)._custom.offset = sideOptions.offset;
        this.add(name, sideLine as any);
      }
    }
  }
    ...
    if (this.options.side != null) {
      ...
      for (let i = 0; i < pCount; i += 1) {
        ...
        if (this.elements[name] != null) {
          ...
          (this.elements[name] as any).updateLabel(rotationOffset);
          if (wasHidden) {
  ...
  updatePoints(newPointsIn: Array<Point>, doNotPublishUpdatePoints: boolean = false) {
    ...
    if (this._line != null) {
      (this._line as any).custom.updatePoints({ points: newPoints });
    }
    ...
    if (this.options.side != null) {
      ...
      for (let i = 0; i < pCount; i += 1) {
        ...
        if (this.elements[name] != null) {
          ...
          // }
          if (this.reverse) {
            (this.elements[name] as any).setEndPoints(
              newPoints[j], newPoints[i], (this.elements[name] as any)._custom.offset,
            );
          } else {
            (this.elements[name] as any).setEndPoints(
              newPoints[i], newPoints[j], (this.elements[name] as any)._custom.offset,
            );
          }
          // if (wasHidden) {
          //   this.elements[name].hide();
  ...
  updateAngles() {
    ...
    for (let i = 0; i < pCount; i += 1) {
      ...
      if (this.elements[name] != null) {
        ...
        (this.elements[name] as any).setAngle({
          p1: this.points[p1],
          p2: this.points[p2],
          p3: this.points[p3],
        });
  ...
  updateRotation(rotationOffset: number = 0) {
    let i = 0;
    let angle = (this as any)[`_angle${i}`];
    while (angle != null) {
      angle.update(this.getRotation() + rotationOffset);
      i += 1;
      angle = (this as any)[`_angle${i}`];
    }

    i = 0;
    let side = (this as any)[`_side${i}${i + 1}`];
    while (side != null) {
      side.updateLabel(this.getRotation() + rotationOffset);
      i += 1;
      side = (this as any)[`_side${i}${i + 1}`];
    }
    side = (this as any)[`_side${i}${0}`];
    if (side != null) {
      side.updateLabel(this.getRotation() + rotationOffset);
    }
  }
  ...
  makeValidTriangle() {
    const angle0 = (this as any)._angle0;
    const angle1 = (this as any)._angle1;
    const angle2 = (this as any)._angle2;
    const side01 = (this as any)._side01;
    const side12 = (this as any)._side12;
    const side20 = (this as any)._side20;
