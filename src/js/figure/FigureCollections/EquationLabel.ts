...
export default class EquationLabel {
  ...
  constructor(
    ...
    if (typeof labelTextOrEquation === 'string') {
      ...
      const formSeries: Array<string> = [];
      labelTextOrEquation.forEach((labelText: string, index: number) => {
      ...
      eqn = collections.equation(joinObjects<any>(
  ...
  setText(text: string) {
    ...
    if (form != null) {
      ...
      (form.elements[key] as any).setText(text);
      form.arrange(
        this.eqn.eqn.scale,
  ...
  getText(): string {
    ...
    if (form != null) {
      ...
      // }
      textToReturn = (form.elements[key] as any).getText();
    }
    return textToReturn;
  }
  ...
  updateRotation(
    ...
    if (DEBUG) {
      const e = (this.eqn as any).parent._debugEllipse;
      if (e != null) {
        e.setPosition(p);
        e.setRotation(r);
      }
  ...
  getOvalOffset(
    ...
    const getR = (a: number, b: number, angle: number) => a * b / Math.sqrt((b * Math.cos(angle)) ** 2 + (a * Math.sin(angle)) ** 2);
    ...
    if (DEBUG) {
      if (this.eqn.parent != null) {
        if ((this.eqn.parent as any)._debugEllipse == null) {
          const e = (this.eqn as any).shapes.ellipse({
            width: 1,
            height: 0.5,
            color: [0, 0, 1, 0.6],
            sides: 50,
          });
          (this.eqn.parent as any).add('debugEllipse', e);
        }
        const e = (this.eqn.parent as any)._debugEllipse;
        if (e != null) {
          e.custom.update({
            width: a * 2,
            height: b * 2,
