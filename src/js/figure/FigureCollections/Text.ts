...
class CollectionsText extends Equation {
  ...
  constructor(
    ...
    super(collections.primitives as any, joinObjects<any>({}, options));
    options.elements = elementsBackup;
    this.xAlign = 'left';
    this.yAlign = 'baseline';
    this.font = joinObjects<any>({}, collections.primitives.defaultFont);
    this.justify = 'left';
    this.modifiers = {};
  ...
  setText(optionsIn: OBJ_FormattedText) {
    ...
    (this.eqn as any).textFont = new FigureFont(equationOptions.textFont as any);
    // console.log(equationOptions)
    ...
    relayout = this.updateElements(
      joinObjects<any>({}, equationOptions.elements, options.elements || {}),
    );

    if (this.updateForms(equationOptions.forms as any, options.reform)) {
      relayout = true;
    }
  ...
  updateElements(
    ...
    Object.keys(elems).forEach((name) => {
      ...
      if (this.elements[name] == null) {
        toAdd[name] = element;
      } else if ((this.elements[name] as any).getText != null) {
        const e = this.elements[name] as any;
  ...
  splitLines(linesIn: Array<Record<string, any> | string> | string) {
    let lines;
    ...
    lines.forEach((lineDefinition, lineIndex) => {
      ...
      if (typeof lineDefinition !== 'string') {
        ...
        if (font != null) {
          lineFont = joinObjects<any>({}, { color: this.color }, this.font, font);
        }
      ...
      const line: Array<Record<string, any>> = [];
      ...
      split.forEach((s: string, i: number) => {
        ...
        if (i % 2 === firstToken) {
          ...
          if (this.modifiers[s] != null) {
            ...
            if (mod.font != null) {
              textFont = joinObjects<any>({}, lineFont, mod.font);
            }
            ...
            if (mod.touch != null) {
              if (mod.touch === true) {
                ...
                if (
                  ...
                  [touchBorder] = getBorder(touchBorder) as any;
                }
              }
            }
            ...
            text = s;
            textFont = joinObjects<any>({}, lineFont, this.accent);
          }
        }
        line.push({
          text,
  ...
  createEquation() {
    ...
    this.lines.forEach((line: Record<string, any>, lineIndex: number) => {
  ...
  createLine(line: Record<string, any>, lineIndex: number) {
    const lineOptions: Record<string, any> = {
