...
function makeStraightLine(
  ...
  const straightLine = (primitives as any).line({
    p1: [0, 0],
...
export default class CollectionsLine extends FigureElementCollection {
  ...
  arrow1: { height: number } | null | undefined;
  arrow2: { height: number } | null | undefined;
  ...
  constructor(
    ...
    if (this.width > 0) {
      ...
      const scaleTransformMethod = (s: number) => new Transform().scale(1, s);
      (straightLine as any).fnMap.add(this.scaleTransformMethodName, scaleTransformMethod);
      this.add('line', straightLine as any);
    }
    ...
    const defaultLabelOptions = {
      ...
      precision: 1,
      update: false,
      font: joinObjects<any>({}, collections.primitives.defaultFont),
    };
    ...
    if (optionsToUse.label !== undefined) {
      ...
      if (typeof optionsToUse.label === 'string' || optionsToUse.label === null) {
        labelOptions = joinObjects<any>({}, defaultLabelOptions, { text: optionsToUse.label });
      } else {
        labelOptions = joinObjects<any>({}, defaultLabelOptions, optionsToUse.label);
      }
    ...
    if (move) {
      const moveOptions = joinObjects<any>({}, defaultMoveOptions, move);
      this.setMovable({
        movable: true,
    ...
    this.fnMap.add('_lengthCallback', (percentage: number) => {
      ...
      toSetup = false;
      const l = ((target as number) - (start as number)) * percentage + (start as number);
      this.setLength(l);
    });
    (this.animations as any).length = (...opt: Array<any>) => {
    ...
    this.fnMap.add('_pulseWidth', (percentage: number, customProperties: Record<string, any>) => {
      const {
    ...
    (this.animations as any).pulseWidth = (...opt: Array<any>) => {
      const o = joinObjects<any>({}, {
        element: this,
        duration: 1,
      }, ...opt);
    ...
    this.animations.customSteps.push({
      step: (this.animations as any).length.bind(this),
      name: 'length',
    });
    this.animations.customSteps.push({
      step: (this.animations as any).pulseWidth.bind(this),
      name: 'pulseWidth',
    });
  ...
  pulseWidthDone() {
    ...
    line.pulseSettings.callback = this.pulseWidthOptions.oldCallback as any;
  }
  ...
  pulseWidth(options?: OBJ_PulseWidth) {
    ...
    if (line != null) {
      line.stopPulsing();
      this.pulseWidthOptions = {
        oldTransformMethod: line.pulseSettings.transformMethod as any,
        oldCallback: line.pulseSettings.callback as any,
      };
      line.pulseSettings.callback = this.pulseWidthDoneCallbackName as any;
      line.pulseSettings.transformMethod = this.scaleTransformMethodName as any;
      line.pulse({
        duration: o.duration,
        scale: o.line,
        frequency: o.frequency,
    ...
    if (label != null) {
      ...
      if (typeof o.label === 'number') {
        labelOptions = joinObjects<any>({}, o, { scale: o.label, callback: done });
      } else {
        labelOptions = joinObjects<any>({}, o, o.label, { callback: done });
      }
  ...
  addArrow(
    ...
    (this as any)[`arrow${index}`] = { height: arrowLength };
   ...
   */
  override setMove(movableOrOptions: any = {}) {
    const defaultOptions = {
      movable: true,
      type: this.move.type,
  ...
  setMultiMovable(middleLengthPercent: number) {
    ...
    if (this._rotPad == null) {
      ...
      (rotPad as any).drawingObject.border = [[]];
    }
    if (this._movePad == null) {
      ...
      (movePad as any).drawingObject.border = [[]];
    }
    // this.hasTouchableElements = true;
    this.isTouchable = false;
    this.isMovable = false;
  ...
  setupLine() {
    ...
    const set = (key: string, x: number) => {
      if ((this as any)[`_${key}`] != null) {
        (this as any)[`_${key}`].transform.updateTranslation([x, 0]);
      }
    };
    let xPosition = 0;
    ...
    if (line) {
      ...
      (line as any).custom.updatePoints({ p1: [0, 0], p2: [straightLineLength, 0], dash: this.dash });
  ...
  pointFromTo(
    ...
    const o = joinObjects<any>({}, defaults, { from, to });
  ...
  grow(options: OBJ_LengthAnimationStep) {
    const o = joinObjects<any>({}, { start: 0 }, options);
    this.animations.new()
      .then((this.animations as any).length(o))
      .start();
  }
