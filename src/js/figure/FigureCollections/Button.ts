...
class ButtonLabel extends EquationLabel {
  constructor(
    ...
    color: TypeColor,
    scale: number = 0.7,
    font: OBJ_Font | null | undefined = undefined,
  ) {
    const xAlign = 'center';
    const yAlign = 'middle';
    super(collections, {
...
class CollectionsButton extends FigureElementCollection {
  ...
  getStateProperty(propertyName: string) {
    let property = (this.states[0] as any)[propertyName];
    for (let i = 0; i <= this._custom.stateIndex; i += 1) {
      if ((this.states[i] as any)[propertyName] != null) {
        property = (this.states[i] as any)[propertyName];
      }
    }
    return property;
  }
    ...
    if (this.states.length > 0) {
      ...
      if (this._label != null) {
        ...
        if (labelText != null && typeof labelText !== 'object') {
          if ((this._label as any).eqn.forms[labelText] != null) {
            (this._label as any).showForm(labelText);
          } else {
