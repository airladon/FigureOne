...
class CollectionsRectangle extends FigureElementCollection {
  ...
  constructor(
    ...
    this.animations.surround = (...opt: any[]) => {
      const o = joinObjects<any>({}, { element: this, space: 0 }, ...opt);
      ...
      o.callback = (percentage: number) => {
        if (toSetup) {
          if (o.start == null) {
            ...
            [startPosition, startWidth, startHeight] = this.getSurround(o.start, o.space) as [Point, number, number];
          }
          if (o.target == null) {
            ...
            [targetPosition, targetWidth, targetHeight] = this.getSurround(o.target, o.space) as [Point, number, number];
          }
  ...
  addButtonBehavior(onClickColors: {
    ...
    const click = (name: string) => {
      const element = this.elements[name] as any;
      if (element == null) {
        return;
      }
      const currentColor = element.color.slice();
      let targetColor: TypeColor | undefined;
      if (Array.isArray(onClickColors)) {
        targetColor = onClickColors.slice() as TypeColor;
      } else if (onClickColors !== true && (onClickColors as any)[name] != null) {
        targetColor = (onClickColors as any)[name].slice();
      }
  ...
  surround(
    ...
    this.setSurround(position as Point, width as number, height as number);
  }
  ...
  setLabel(text: string | OBJ_FormattedText) {
    if (this._label != null) {
      ...
      (this._label as any).setText(textToUse);
    }
    this.animateNextFrame();
  }
  ...
  getLabel() {
    if (this._label != null) {
      let out = '';
      const form = (this._label as any).getCurrentForm();
      if (form == null) {
        return '';
      }
