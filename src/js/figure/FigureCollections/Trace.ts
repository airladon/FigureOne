...
class CollectionsTrace extends FigureElementCollection {
  ...
  update(points: Array<TypeParsablePoint>) {
    ...
    if (this.line != null) {
      this.addLine(this.line as any);
    }
    if (this.markers != null) {
      this.markers.copy = [] as any;
  ...
  intersect(p1: Point, p2: Point): { result: boolean; intersect?: Array<Point> } {
    if (p1.x < 0 && p2.x < 0) {
      return { result: false };
    }
    if (p1.y < 0 && p2.y < 0) {
    ...
    bounds.forEach((bound) => {
      ...
      if (i.onLines) {
        result = true;
        intersect.push(i.intersect as Point);
      }
    });
  ...
  updatePoints() {
    ...
    const drawIndexes: Array<number> = [];
    for (let i = 0; i < this.points.length; i += 1) {
    ...
    const inX: Array<boolean> = [];
    const inY: Array<boolean> = [];
    ...
    for (let i = 0; i < this.drawPoints.length; i += 1) {
      ...
      // Next point leaving the axes
        ...
        const intersectResult = this.intersect(this.drawPoints[i], this.drawPoints[i + 1]);
        if (intersectResult.result && intersectResult.intersect) {
          polyline.push(...intersectResult.intersect);
        }
        this.polylines.push(polyline);
        polyline = [];
        inLine = false;
      ...
      // the line between points may cross over into the axes
        ...
        const intersectResult = this.intersect(this.drawPoints[i], this.drawPoints[i + 1]);
        if (intersectResult.result && intersectResult.intersect) {
          this.polylines.push(intersectResult.intersect.map(p => p._dup()));
        }
      }
    }
  }
  ...
  addLine(options: OBJ_LineStyleSimple) {
    ...
    this.updatePoints();
    this.line = joinObjects<any>({}, defaultOptions, options);
    this.polylines.forEach((points, index) => {
      const line = this.collections.primitives.polyline(joinObjects<any>({}, this.line, { points }));
