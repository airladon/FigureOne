...
export default class FigureCollections {
  ...
  collection(
    ...
    if (transformOrPointOrOptions instanceof Point) {
      ...
      optionsToUse = joinObjects<any>({}, defaultOptions, ...moreOptions);
    } else if (transformOrPointOrOptions instanceof Transform) {
      defaultOptions.transform = transformOrPointOrOptions._dup();
      optionsToUse = joinObjects<any>({}, defaultOptions, ...moreOptions);
    } else {
      optionsToUse = joinObjects<any>({}, defaultOptions, transformOrPointOrOptions, ...moreOptions);
    }
  ...
  line(...options: Array<COL_Line>) {
    const optionsToUse = joinObjects<any>({}, { timeKeeper: this.primitives.timeKeeper }, ...options);
  ...
  rectangle(...options: Array<COL_Rectangle>) {
    return new CollectionsRectangle(
      this, joinObjects<any>({}, { timeKeeper: this.primitives.timeKeeper }, ...options),
    );
  }
  ...
  addEquation(
    ...
    name: string,
    options: EQN_Equation = {} as any,
  ) {
    const equation = new Equation(this.primitives, options);
    parent.add(name, equation as any);
    return equation;
  }
  ...
  cursor(
    ...
    const up = this.primitives.polygon(
      joinObjects<any>({}, polygon, { line: { width: options.width } }),
    );
    const down = this.primitives.polygon(joinObjects<any>({}, polygon));
    cursor.add('up', up);
