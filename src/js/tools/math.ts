const roundNum = (value: number, precision: number = 5): number => {
  ...
  //   result = 0;
  // }
  const objectIsPolyfill = (x: number, y: number): boolean => {
    if (x === y) {
...
interface TypeRoundObject {
  round?: (precision: number) => TypeRoundObject;
}
...
| [
  ...
  if (Array.isArray(arrayOrValue)) {
    return arrayOrValue.map(elem => round(elem, precision)) as T;
  }
  if (typeof arrayOrValue === 'number') {
    result = roundNum(arrayOrValue, precision);
  } else if (arrayOrValue != null && (arrayOrValue as TypeRoundObject).round != null) {
    result = (arrayOrValue as TypeRoundObject).round!(precision);
  } else {
    result = arrayOrValue;
  }
  return result;
...
const linear = (percentTime: number, invert: boolean = false): number => {
  if (invert) {
    return percentTime;
  }
  return percentTime;
...
const easeinout = (percentTime: number, invert: boolean = false): number => {
  if (invert) {
    if (percentTime === 0.5) {
      return 0.5;
    }
...
function easeout(percentTime: number, invert: boolean = false): number {
  if (invert) {
    if (percentTime === 0) {
      return 0;
    }
...
function easein(percentTime: number, invert: boolean = false): number {
  if (invert) {
    if (percentTime === 1) {
      return 1;
    }
...
function rand2D(minX: number, minY: number, maxX: number, maxY: number): { x: number; y: number } {
  return {
    x: rand(minX, maxX),
    y: rand(minY, maxY),
  };
