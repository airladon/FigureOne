...
const makeShape = (options, reverseLight) => {
  ...
  // console.log(scene)
  return tools.misc.joinObjects({ name, make: 'cone' }, {
...
const shapes = [
  ...
  makeShape({ line: [[0, -0.2, 0], [0.5, -0.2, 0]] }),
  makeShape({ line: [[0, 0, 0], [0, -0.4, 0]] }),
  makeShape({ line: null, length: 0.7 }),
  makeShape({ rotation: Math.PI / 10 }),
  makeShape({ normals: 'curve', sides: 20 }),
  makeShape({ normals: 'curve', sides: 20, line: [[0, 0, 0], [0, -0.4, 0]] }),
  makeShape({ lines: true }),
  makeShape({}, true),
  makeShape({ line: [[0, 0, 0], [0, -0.4, 0]] }, true),
];
figure.add(shapes);
