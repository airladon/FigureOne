...
function __finish(__figure) {
  ...
  // }
  const stateTimes = __figure.recorder.states.diffs.map(d => d[0]);
  const deltaTimes = stateTimes.map((t, i) => {
    if (i > 0) {
      return Math.round((t[i] - t[i - 1]) * 10000) / 10000;
    }
    return stateTimes[0];
  });
  deltaTimes.forEach((t) => {
    __steps.push([t]);
  });
  console.log(__steps);
